...
void StgChecker::useHitArea(int n) {
	if (_pHitAreaBoxs == NULL) {
		...
		throw_GgafCriticalException("StgChecker::useHitArea HitAreaBoxs‚ÍŠù‚É new ‚³‚ê‚Ä‚¢‚Ü‚·B");
	}
}

void StgChecker::setHitArea(int prm_index, int x1, int y1, int z1, int x2, int y2, int z2, bool rotX, bool rotY, bool rotZ) {
	if (_pHitAreaBoxs == NULL) {
		throw_GgafCriticalException("StgChecker::setHitArea ‚Ü‚¸ useHitArea ‚ðŽÀs‚µ‚ÄA—v‘f”‚ðéŒ¾‚µ‚Ä‚­‚¾‚³‚¢B");
	} else {
