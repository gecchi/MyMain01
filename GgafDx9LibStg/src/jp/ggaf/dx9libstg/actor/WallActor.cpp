...
WallActor::WallActor(const char* prm_name, const char* prm_model) :
    ...
    _pCollisionChecker->makeCollision(2); //0:BOX用当たり判定、1:プリズム用当たり判定
    _pCollisionChecker->setColliAAB(0, 0,0,0, 0,0,0);
    _pCollisionChecker->setColliAAPrism(1, 0,0,0, 0,0,0, 0);
...
void WallActor::config(WalledSectionScene* prm_pWalledSectionScene, int prm_pos_prism, int prm_wall_draw_face, int* prm_aColliBoxStretch) {
    ...
    if(prm_pos_prism == 0) {
        _pCollisionChecker->disable(1); //プリズム側当たり判定OFF
        ...
        if (prm_aColliBoxStretch[0] == 0) {
            ...
            _pCollisionChecker->setColliAAPrism(1, -(_wall_dep/2)    - (_wall_dep    * (prm_aColliBoxStretch[FACE_B_IDX]-1)),
                                                   -(_wall_height/2) - (_wall_height * (prm_aColliBoxStretch[FACE_D_IDX]-1)),
                                                   -(_wall_width/2)  - (_wall_width  * (prm_aColliBoxStretch[FACE_E_IDX]-1)),
                                                    (_wall_dep/2)    + (_wall_dep    * (prm_aColliBoxStretch[FACE_F_IDX]-1)),
                                                    (_wall_height/2) + (_wall_height * (prm_aColliBoxStretch[FACE_A_IDX]-1)),
