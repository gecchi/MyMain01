...
PreDrawScene::PreDrawScene(const char* prm_name) : DefaultScene(prm_name) {
    ...
    wantActor(id, CubeMapMeshTestActor);           id++;
    wantActor(id, CubeMapMeshSetTestActor);        id++;
    wantActor(id, CubeMapMorphMeshTestActor);      id++;
    wantActor(id, DefaultD3DXAniMeshTestActor);   id++;
//    wantActor(id, DefaultBoardTestActor);          id++;
//    wantActor(id, DefaultBoardSetTestActor);       id++;
    wantActor(id, DefaultMeshTestActor);           id++;
    wantActor(id, DefaultMeshSetTestActor);        id++;
    wantActor(id, DefaultMorphMeshTestActor);      id++;
//    wantActor(id, DefaultPointSpriteTestActor);    id++;
//    wantActor(id, DefaultSpriteTestActor);         id++;
//    wantActor(id, DefaultSpriteSetTestActor);      id++;
    wantActor(id, HoshiBoshiTestActor);            id++;
    wantActor(id, LaserChipTestActor);             id++;
    wantActor(id, SingleLaserTestActor);           id++;
    wantActor(id, SpriteMeshTestActor);            id++;
    wantActor(id, SpriteMeshSetTestActor);         id++;
    wantActor(id, WallAABTestActor);               id++;
    wantActor(id, WallAAPrismTestActor);           id++;
    order_id_end_ = id - 1;
    useProgress();
