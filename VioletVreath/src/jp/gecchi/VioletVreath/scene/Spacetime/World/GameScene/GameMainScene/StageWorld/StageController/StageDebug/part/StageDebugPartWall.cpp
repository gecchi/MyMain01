...
StageDebugPartWall::StageDebugPartWall(const char* prm_name) :
    ...
    getSceneChief()->appendChild(pDepo_wall);
...
void StageDebugPartWall::processBehavior() {
	...
	if (getBehaveingFrame() == _paFrame_NextEvent[_cnt_event]) {
		switch (getBehaveingFrame()) {
			...
			case 2000: {
				getSceneChief()->appendChild(receiveActor(50000000));
				break;
			}
			case 5400: {
			...
			case 6000: {
				getSceneChief()->appendChild(receiveActor(50000001));
				break;
			}
			default :
				break;
...
void StageDebugPartWall::onFinishedAllSection() {
    _TRACE_(" StageDebugPartWall::onFinishedAllSection()  CommonScene のスクロールを解除します。");
