    ...
    switch (pProg->get()) {
        ...
        case PROG_INPUT: {
            ...
            if (pProg->hasJustChanged()) {
                ...
                pLabelInputedName_->callVecDriver()->setRollPitchYawFaceAngVelo(1700, 1500, 1000);
            }
            pLabelInputedName_->callVecDriver()->behave();
        ...
        case PROG_DONE_DISP: {
            ...
            if (pProg->hasJustChanged()) {
                ...
                pLabelInputedName_->callVecDriver()->turnRzRyFaceAngTo(D0ANG, D0ANG,
                                                                   8000, 0,
                                                                   TURN_COUNTERCLOCKWISE, false);//急いで正常姿勢へ
                pLabelInputedName_->callVecDriver()->rollFaceAngTo(D0ANG, 8000, 0, TURN_COUNTERCLOCKWISE);
                //ここでハイスコア更新処理
