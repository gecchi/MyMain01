...
Stage01::Stage01(const char* prm_name) : Stage(prm_name) {
    ...
    pScene_Stage01PartController_ = NEW Stage01PartController("Stage01Ctrl");
    pScene_Stage01PartController_->inactivate();
    addSubLast(pScene_Stage01PartController_);
    Sleep(2);
    ...
    switch (_pProg->get()) {
        ...
        case Stage::PROG_BEGIN: {
            if (_pProg->getFrameInProgress() == 180) { //ステージ１開始！
                ...
                pScene_Stage01PartController_->activate();
...
void Stage01::onCatchEvent(hashval prm_no, void* prm_pSource) {
    if (prm_no == EVENT_STG01_CONTROLLER_WAS_END ) {
        ...
        pScene_Stage01PartController_->end(60*60);
