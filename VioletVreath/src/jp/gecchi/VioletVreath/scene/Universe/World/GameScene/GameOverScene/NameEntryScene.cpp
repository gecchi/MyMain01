...
void NameEntryScene::initialize() {
    _DTRACE_("NameEntryScene::initialize()");
}
    ...
    switch (pProg->get()) {
        ...
        case NameEntryScene::PROG_PRE_DISP: {
            ...
            if (pProg->isJustChanged()) {
                _DTRACE_("NameEntryScene::processBehavior() Prog has Just Changed (to NameEntryScene::PROG_PRE_DISP)");
                pWorldBound_->fadein();
            }
        ...
        case NameEntryScene::PROG_INPUT: {
            //##########  ネームエントリー  ##########
            if (pProg->isJustChanged()) {
                _DTRACE_("NameEntryScene::processBehavior() Prog has Just Changed (to NameEntryScene::PROG_INPUT)");
        ...
        case NameEntryScene::PROG_DONE_DISP: {
            //##########  ネームエントリー完了後の画面表示  ##########
            if (pProg->isJustChanged()) {
                _DTRACE_("NameEntryScene::processBehavior() Prog has Just Changed (to NameEntryScene::PROG_DONE_DISP)");
        ...
        case NameEntryScene::PROG_FINISH: {
            if (pProg->isJustChanged()) {
                _DTRACE_("NameEntryScene::processBehavior() Prog has Just Changed (to NameEntryScene::PROG_FINISH)");
                _DTRACE_("おわりじゃよ！");
...
void NameEntryScene::onCatchEvent(hashval prm_no, void* prm_pSource) {
    ...
    if (prm_no == EVENT_MENU_NAMEENTRY_DONE) {
        _DTRACE_("NameEntryScene::onCatchEvent(EVENT_MENU_NAMEENTRY_DONE)");
