    ...
    switch (_pProg->get()) {
         ...
         case PROG_MOVE01: {
             if (_pProg->isJustChanged()) {
                 //滞留ポイントへGO!
                 velo mv_velo = 1;//R1F_EnemyThagoras_MvVelo(G_RANK);
         ...
         case PROG_MOVE02: {
             ...
             if (_pProg->getFrameInProgress() == 180) {
                 //自機の方に向いたら敵弾発射！
                 int shot_num = 1;//RF_EnemyThagoras_ShotWay(G_RANK); //弾数、ランク変動
                 velo shot_velo = 1;//RF_EnemyThagoras_ShotMvVelo(G_RANK); //弾速、ランク変動
