...
EnemyGeria::EnemyGeria(const char* prm_name) :
    ...
    pSeTx->set(SE_FIRE     , "SE_ENEMY_FIRE_SHOT_001");     //発射
...
void EnemyGeria::onActive() {
    ...
    frame_when_shot_ = 0;
    GgafDx::LocoVehicle* pLocoVehicle = getLocoVehicle();
    velo_mv_begin_ = pLocoVehicle->getMvVelo(); //初期移動速度を保存
...
void EnemyGeria::processBehavior() {
    ...
    switch (pPhase->getCurrent()) {
        ...
        case PHASE_FIRE: {  //発射
            if (pPhase->hasJustChanged()) {
                pLocoVehicle->setMvVelo(PX_C(3)); //減速
                pLocoVehicle->rollFaceAngTo(D180ANG, D_ANG(3), 0, TURN_CLOCKWISE); //予備動作のぐるっと回転
            }
