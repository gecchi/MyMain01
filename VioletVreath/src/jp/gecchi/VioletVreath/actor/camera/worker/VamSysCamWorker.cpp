...
void VamSysCamWorker::processBehavior() {
    ...
    const bool isPressed_VV_VB_UP    = pVbPlay->isPressed(0, VV_VB_UP);
    const bool isPressed_VV_VB_DOWN  = pVbPlay->isPressed(0, VV_VB_DOWN);
    const bool isPressed_VV_VB_LEFT  = pVbPlay->isPressed(0, VV_VB_LEFT);
    const bool isPressed_VV_VB_RIGHT = pVbPlay->isPressed(0, VV_VB_RIGHT);

    const bool isPressed_VV_VB_VIEW = pVbPlay->isPressed(0, VV_VB_VIEW);
    const bool isDoublePushedDown_VV_VB_VIEW = pVbPlay->isDoublePushedDown(0, VV_VB_VIEW);
    const bool isPressed_VV_VB_VIEW_UP    = pVbPlay->isPressed(0, VV_VB_VIEW_UP)    ||  (isPressed_VV_VB_VIEW && isPressed_VV_VB_UP   );
    const bool isPressed_VV_VB_VIEW_DOWN  = pVbPlay->isPressed(0, VV_VB_VIEW_DOWN)  ||  (isPressed_VV_VB_VIEW && isPressed_VV_VB_DOWN );
    const bool isPressed_VV_VB_VIEW_LEFT  = pVbPlay->isPressed(0, VV_VB_VIEW_LEFT)  ||  (isPressed_VV_VB_VIEW && isPressed_VV_VB_LEFT );
    const bool isPressed_VV_VB_VIEW_RIGHT = pVbPlay->isPressed(0, VV_VB_VIEW_RIGHT) ||  (isPressed_VV_VB_VIEW && isPressed_VV_VB_RIGHT);
    ...
    if (!returning_cam_pos_ &&
        (isPressed_VV_VB_VIEW_UP || isPressed_VV_VB_VIEW_DOWN || isPressed_VV_VB_VIEW_LEFT || isPressed_VV_VB_VIEW_RIGHT)
    ) {
        double vx = 0;
        double vy = 0;
        if (isPressed_VV_VB_VIEW_UP) {
            if (isPressed_VV_VB_VIEW_LEFT) {
                vx = 0.7071067811865475;
                vy = -0.7071067811865475;
            } else if (isPressed_VV_VB_VIEW_RIGHT) {
                vx = 0.7071067811865475;
                vy = 0.7071067811865475;
            } else {
                vx = 1;
        ...
        } else if (isPressed_VV_VB_VIEW_DOWN) {
            if (isPressed_VV_VB_VIEW_LEFT) {
                vx = -0.7071067811865475;
                vy = -0.7071067811865475;
            } else if (isPressed_VV_VB_VIEW_RIGHT) {
        ...
        } else if (isPressed_VV_VB_VIEW_LEFT) {
            vx = 0;
            vy = -1;
        } else if (isPressed_VV_VB_VIEW_RIGHT) {
            vx = 0;
    ...
    if ((pos_vam_camera_prev_ != pos_vam_camera_) || isDoublePushedDown_VV_VB_VIEW) {
