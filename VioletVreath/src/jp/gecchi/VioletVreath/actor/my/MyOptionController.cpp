...
MyOptionController::MyOptionController(const char* prm_name) :
  GgafDxGeometricActor(prm_name, NULL, NULL) {
    ...
    _pKurokoB->forceVxMvVeloRange(-_renge, _renge);
    _pKurokoB->forceVyMvVeloRange(-_renge, _renge);
    _pKurokoB->forceVzMvVeloRange(-_renge, _renge);

    _pKurokoB->forceVxMvAcceRange(-_renge / 30, _renge / 30);
    _pKurokoB->forceVyMvAcceRange(-_renge / 30, _renge / 30);
    _pKurokoB->forceVzMvAcceRange(-_renge / 30, _renge / 30);
    ...
    _papMyOption[0]->config(60000, D0ANG, 0, 1000);
    ...
    _papMyOption[1]->config(60000, D90ANG, 0, 1000);
    ...
    _papMyOption[2]->config(60000, D180ANG, 0, 1000);
    ...
    _papMyOption[3]->config(60000, D270ANG, 0, 1000);
    ...
    _papMyOption[4]->config(120000, D0ANG, 0, -1500);
    ...
    _papMyOption[5]->config(120000, D90ANG, 0, -1500);
    ...
    _papMyOption[6]->config(120000, D180ANG, 0, -1500);
    ...
    _papMyOption[7]->config(120000, D270ANG, 0, -1500);
...
void MyOptionController::onReset() {
    ...
    _pKurokoA->forceRyMvAngVeloRange(-1*_angVelo_Turn, _angVelo_Turn);
    _pKurokoA->forceRzMvAngVeloRange(-1*_angVelo_Turn, _angVelo_Turn);
...
//    }
    ...
    if (VB_PLAY->isDoublePushedDown(VB_OPTION,8,8)) {
        ...
        if (VB_PLAY->isBeingPressed(VB_UP)) {
            _pKurokoA->addRzMvAng(_angVelo_Turn);
        }
        if (VB_PLAY->isBeingPressed(VB_DOWN)) {
            _pKurokoA->addRzMvAng(-_angVelo_Turn);
        }
        if (VB_PLAY->isBeingPressed(VB_RIGHT)) {
            _pKurokoA->addRyMvAng(_angVelo_Turn);
        }
    ...
    if (_is_free_from_myship_mode) {
        if (VB_PLAY->isBeingPressed(VB_OPTION) && _is_handle_move_mode) {
            ...
            //そのうちのオプション移動速度のみを設定。
            _pKurokoA->setMvVelo(GgafDxUtil::COS[_papMyOption[0]->_angExpanse/ SANG_RATE] * _veloOptionsMv);
    ...
    _pDirectionVector->_pKurokoA->setRzRyMvAng(_pKurokoA->_angRzMv, _pKurokoA->_angRyMv);
...
void MyOptionController::adjustDefaltAngPosition(frame prm_spent_frame) {
    if (_now_option_num <= 4) {
        for (int i = 0; i < _now_option_num; i++) {
            _papMyOption[i]->adjustAngPosition((D360ANG/_now_option_num)*i,prm_spent_frame);
        }
    } else if (_now_option_num > 4) {
        for (int i = 0; i < 4; i++) {
            _papMyOption[i]->adjustAngPosition((D360ANG/4)*i, prm_spent_frame);
        }
        for (int i = 4; i < _now_option_num; i++) {
            _papMyOption[i]->adjustAngPosition((D360ANG/(_now_option_num-4))*(i-4), prm_spent_frame);
        }
    }
}
