...
ShotMagic::ShotMagic(const char* prm_name, int* prm_pMP)
    : Magic(prm_name, prm_pMP,
            ...
            60*1    , 1.5, 0.9,   //基本詠唱時間  , ＋１レベル毎の詠唱時間増加率, 飛びレベル時の詠唱時間削減率
            30      , 1.5, 0.9,   //基本発動時間  , ＋１レベル毎の発動時間増加率, 飛びレベル時の発動時間削減率
            60*60*10, 0.8,        //基本持続時間  , ＋１レベル毎の持続時間の乗率
...
void ShotMagic::processCastBegin(int prm_now_level, int prm_new_level) {
    if (prm_new_level > prm_now_level) {
        ...
        pEffect_->getVecDriver()->setFaceAngVelo(AXIS_Z, 100);
...
void ShotMagic::processInvokeBegin(int prm_now_level, int prm_new_level) {
    if (prm_new_level > prm_now_level) {
        pEffect_->setScale(1000);
        pEffect_->getVecDriver()->setFaceAngVelo(AXIS_Z, -3000);
    }
}
