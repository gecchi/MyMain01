...
r_time_of_invoking_lv_diff_base_(prm_r_time_of_invoking_lv_diff_base) {
    ...
    _DTRACE_("Magic::Magic "<<getName()<<" のレベルアップ時の数値");
    ...
    for (int now_lv = 0; now_lv <= MMETER_MAX_LEVEL; now_lv++) {
        for (int target_lv = now_lv; target_lv <= MMETER_MAX_LEVEL; target_lv++) {
             _DTRACE_("Magic::Magic["<<getName()<<"] level_up_cost_["<<now_lv<<"→"<<target_lv<<"]="<<level_up_cost_[now_lv][target_lv]);
        }
    }
    for (int now_lv = 0; now_lv <= MMETER_MAX_LEVEL; now_lv++) {
        for (int target_lv = now_lv; target_lv <= MMETER_MAX_LEVEL; target_lv++) {
             _DTRACE_("Magic::Magic["<<getName()<<"] level_up_time_of_casting_["<<now_lv<<"→"<<target_lv<<"]="<<level_up_time_of_casting_[now_lv][target_lv]);
        }
    }
    for (int now_lv = 0; now_lv <= MMETER_MAX_LEVEL; now_lv++) {
        for (int target_lv = now_lv; target_lv <= MMETER_MAX_LEVEL; target_lv++) {
            _DTRACE_("Magic::Magic["<<getName()<<"] level_up_time_of_invoking_["<<now_lv<<"→"<<target_lv<<"]="<<level_up_time_of_invoking_[now_lv][target_lv]);
        }
    }
#endif
...
void Magic::onReset() {
    _DTRACE_("Magic::onReset() ["<<getName()<<"] ");
    new_level_  = 0;
    last_level_ = 0;
    level_      = 0;
    prev_frame_level_ = level_;
...
int Magic::cast(int prm_new_level) {
    ...
    switch (last_cast) {
        ...
        case MAGIC_CAST_CANCEL: {
            ...
            //詠唱中止(キャンセル)扱いになる。
            _DTRACE_("Magic::cast("<<prm_new_level<<") ["<<getName()<<"] 判定→MAGIC_CAST_CANCEL!");
        ...
        case MAGIC_CAST_OK_LEVELUP: {
            ...
            _DTRACE_("Magic::cast("<<prm_new_level<<") ["<<getName()<<"] 判定→MAGIC_CAST_OK_LEVELUP!");
            new_level_nextframe_ = prm_new_level;
        ...
        case MAGIC_CAST_LEVELDOWN: {
            ...
            _DTRACE_("Magic::cast("<<prm_new_level<<") ["<<getName()<<"] 判定→MAGIC_CAST_LEVELDOWN!");
            new_level_nextframe_ = prm_new_level;
            pProg->change(STATE_CASTING);
            break;
        }
        case MAGIC_CAST_OK_CANCEL_AND_LEVELUP: {
            ...
            _DTRACE_("Magic::cast("<<prm_new_level<<") ["<<getName()<<"] 判定→MAGIC_CAST_OK_CANCEL_AND_LEVELUP!");
            new_level_nextframe_ = prm_new_level;
        ...
        case MAGIC_CAST_CANCEL_AND_LEVELDOWN: {
            ...
            _DTRACE_("Magic::cast("<<prm_new_level<<") ["<<getName()<<"] 判定→MAGIC_CAST_CANCEL_AND_LEVELDOWN!");
            new_level_nextframe_ = prm_new_level;
            pProg->change(STATE_RE_CASTING);
            break;
        }
...
int Magic::invoke(int prm_new_level) {
    ...
    switch (last_invoke) {
        ...
        case MAGIC_INVOKE_NG_MP_IS_SHORT: {
            _DTRACE_("Magic::invoke("<<prm_new_level<<") ["<<getName()<<"] 判定→MAGIC_INVOKE_NG_MP_IS_SHORT!");
            pProg->change(STATE_NOTHING);
            break;
        }
        case MAGIC_INVOKE_OK_LEVELUP: {
            _DTRACE_("Magic::invoke("<<prm_new_level<<") ["<<getName()<<"] 判定→MAGIC_INVOKE_OK_LEVELUP!");
            new_level_nextframe_ = prm_new_level;
            pProg->change(STATE_INVOKING);
            break;
        }
        case MAGIC_INVOKE_OK_LEVELDOWN: {
            _DTRACE_("Magic::invoke("<<prm_new_level<<") ["<<getName()<<"] 判定→MAGIC_INVOKE_OK_LEVELDOWN!");
            new_level_nextframe_ = prm_new_level;
            pProg->change(STATE_INVOKING);
            break;
        }
...
int Magic::effect(int prm_level) {
    ...
    switch (last_effect) {
        case MAGIC_EFFECT_NG_MP_IS_SHORT: {
            ...
            //ありうる
            _DTRACE_("Magic::effect("<<prm_level<<") ["<<getName()<<"] 判定→MAGIC_EFFECT_NG_MP_IS_SHORT、change(STATE_NOTHING)");
            pProg->change(STATE_NOTHING);
            break;
        }
        case MAGIC_EFFECT_NOTHING: {
            if (prm_level == 0) {
                _DTRACE_("Magic::effect("<<prm_level<<") ["<<getName()<<"] 判定→MAGIC_EFFECT_NOTHING、なにもしません");
            } else {
        ...
        case MAGIC_EFFECT_OK_LEVELUP: {
            _DTRACE_("Magic::effect("<<prm_level<<") ["<<getName()<<"] 判定→MAGIC_EFFECT_OK_LEVELUP、change(STATE_EFFECT_START)");
            //レベル更新
        ...
        case MAGIC_EFFECT_OK_LEVELDOWN: {
            _DTRACE_("Magic::effect("<<prm_level<<") ["<<getName()<<"] 判定→MAGIC_EFFECT_OK_LEVELDOWN、change(STATE_RE_EFFECT)");
            //レベル更新
            last_level_nextframe_ = level_;
            level_nextframe_ = prm_level;
            pProg->change(STATE_RE_EFFECT); //現在 STATE_EFFECT_START のため
            break;
        }
...
void Magic::processBehavior() {
    ...
    switch (prog) {
        ...
        case STATE_NOTHING: {
            if (pProg->isJustChanged()) {
                if (pProg->isJustChangedFrom(STATE_CASTING)) {
                    _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 詠唱キャンセル終了！");
                    processCastingCancel(level_); //コールバック
                } else if (pProg->isJustChangedFrom(STATE_INVOKING)) {
                    _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 発動キャンセル終了！");
        ...
        case STATE_RE_CASTING: {
            _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 詠唱キャンセル再詠唱！");
            processCastingCancel(level_); //コールバック
            pProg->change(STATE_CASTING);
            break;
        }
        ...
        case STATE_CASTING: {
            if (pProg->isJustChanged()) { //詠唱開始
                ...
                _DTRACE_("Magic::processBehavior() ["<<getName()<<"] STATE_CASTING begin new_level_="<<new_level_<<" level_="<<level_<<" time_of_next_state_="<<time_of_next_state_<<"");
            ...
            if (level_ > new_level_ || pProg->getFrameInProgress() >= time_of_next_state_) {
                ...
                _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 詠唱(STATE_CASTING)ステータス終了！");
            }
            break;
        }
        ...
        case STATE_INVOKING: {
            if (pProg->isJustChanged()) { //発動開始
                ...
                _DTRACE_("Magic::processBehavior() ["<<getName()<<"] STATE_INVOKING begin new_level_="<<new_level_<<" level_="<<level_<<" time_of_next_state_="<<time_of_next_state_<<"");
            ...
            if (level_ > new_level_ || pProg->getFrameInProgress() >= time_of_next_state_) {
                ...
                _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 発動(STATE_INVOKING)ステータス終了！");
            }
            break;
        }
        ...
        case STATE_EFFECT_START: {
            if (pProg->isJustChanged()) { //持続開始
                _DTRACE_("Magic::processBehavior() ["<<getName()<<"] STATE_EFFECT_START begin");

                processEffectBegin(last_level_, level_); //効果持続開始コールバック

                //MP消費計算
                if (last_level_ < level_) {
                    _DTRACE_("Magic::processBehavior() ["<<getName()<<"] レベルアップだった。last_level_="<<last_level_<<" level_="<<level_);
                    //レベルアップだった場合
                    ...
                    *pMP_ -= level_up_cost_[last_level_][level_]; //MP消費
                    _DTRACE_("Magic::processBehavior() ["<<getName()<<"] "<<level_up_cost_[last_level_][level_]<<"MP消費("<<((*pMP_)+level_up_cost_[last_level_][level_])<<"→"<<(*pMP_)<<")");
                    if (*pMP_ < 0) {
                        *pMP_ = 0;
                    }
                } else if (last_level_ > level_) {
                    _DTRACE_("Magic::processBehavior() ["<<getName()<<"] レベルダウンだった。last_level_="<<last_level_<<" level_="<<level_);
                    //レベルダウンだった場合
                    if (keep_cost_base_ <= 0) { //維持コストがかからない魔法の場合は
                        if (lvinfo_[last_level_].time_of_effect_ > 0) {
                            ...
                            *pMP_ += rmp;
                            _DTRACE_("Magic::processBehavior() ["<<getName()<<"] "<<rmp<<"MP還元("<<((*pMP_)-rmp)<<"→"<<(*pMP_)<<")");
                    ...
                    //last_level_ == level_
                    throwGgafCriticalException("Magic::processBehavior() ["<<getName()<<"] 新旧同じレベルでの発動はあり得ないはずです。level_="<<level_<<" last_level_="<<last_level_<<"");
                }
    ...
    if (temp_hold_status_ != -1) {
        ...
        if (temp_hold_status_ == STATE_CASTING) {
            _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 持続時間満期レベルダウン時に詠唱中だったのでここで自動再詠唱！ cast("<<temp_hold_new_level_<<")");
            cast(temp_hold_new_level_); //再詠唱
        } else if (temp_hold_status_  == STATE_INVOKING) {
            _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 持続時間満期レベルダウン時に詠唱中だったのでここで自動再発動！ invoke("<<temp_hold_new_level_<<")");
    ...
    if (level_ > 0) {
        if (pProg->isJustChangedTo(STATE_EFFECT_START) && time_of_effect_base_ == 0) {
            ...
            effect(0);
            _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 即効性魔法のため、いきなり持続終了。last_level_="<<last_level_<<" level_="<<level_);
            break;
        }
            ...
            *pMP_ += -1*lvinfo_[level_].keep_cost_; //維持コスト減少
            ...
            if (*pMP_ <= 0) {
                ...
                *pMP_ = 0;
                ...
                effect(0);
                _DTRACE_("Magic::processBehavior() ["<<getName()<<"] MP枯渇による持続終了。");
                break;
            }
        }
        ...
        if (lvinfo_[level_].remainingtime_of_effect_ == 0) { //満期？
            //持続時間満期でレベルダウン処理
             _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 持続時間満期処理 lv="<<level_<<"→"<<(level_-1)<<"");
            ...
            if (prog == STATE_CASTING) {
                _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 効果持続中・・・・、現在詠唱中で持続時間満期処理。effect() 後再詠唱予約");
                temp_hold_status_ = pProg->get(); //そこで、現ステータスを一時退避(やや苦しい・・・)
                temp_hold_new_level_ = new_level_; //詠唱又は発動しようとしていた新レベルも保持
            } else if (prog == STATE_INVOKING) {
                _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 効果持続中・・・・、現在発動中で持続時間満期処理。effect() 後発動予約");
                temp_hold_status_ = pProg->get(); //そこで、現ステータスを一時退避(やや苦しい・・・)
                temp_hold_new_level_ = new_level_; //詠唱又は発動しようとしていた新レベルも保持
            } else {
                //STATE_EFFECT_START
                _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 効果持続中・・・・、普通に持続時間満期処理 (^_^)");
            }
            effect(level_-1);
            _DTRACE_("Magic::processBehavior() ["<<getName()<<"] 持続時間満期で持続終了。");
            break;
        }
    }
    ...
    //_DTRACE_("Magic::processBehavior() F="<<getBehaveingFrame()<<" after pProg->get()="<<pProg->get());
...
magic_point Magic::calcReduceMp(int prm_now_level, int prm_target_down_level) {
    ...
    //_DTRACE_("calcReduceMp!!!");
    //_DTRACE_("prm_now_level="<<prm_now_level<<" prm_target_down_level="<<prm_target_down_level);
    magic_point mp = 0;
    for (int lv = prm_now_level; lv > prm_target_down_level; lv--) {
        ...
        //_DTRACE_(lv << ":lvinfo_["<<lv<<"].cost_="<<(lvinfo_[lv].cost_)<<" ");
        //_DTRACE_(lv << ":utilization_rate="<<utilization_rate<<" ");
        //_DTRACE_(lv << ":lvdiff_[max_level_-(prm_now_level-lv)]=lvdiff_["<<max_level_<<"-("<<prm_now_level<<"-"<<lv<<")]=lvdiff_["<<(max_level_-(prm_now_level-lv))<<"].r_cost_lv_diff_="<<(lvdiff_[max_level_-(prm_now_level-lv)].r_cost_lv_diff_));
        //_DTRACE_(lv << ":+="<<(lvinfo_[lv].cost_ * utilization_rate * lvdiff_[max_level_-(prm_now_level-lv)].r_cost_lv_diff_));
        //_DTRACE_(lv << ":mp="<<mp);
    }
//_DTRACE_("return="<<mp);
    return mp;
