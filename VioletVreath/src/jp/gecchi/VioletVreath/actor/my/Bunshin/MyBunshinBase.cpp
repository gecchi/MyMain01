...
//                    }
                ...
                case VAM_POS_ZRIGHT_UP: {
                    // a = (0, 1, 1)        LEFT RIGHT 回転軸
                    // b = (vX, vY, vZ)
                    // a×b = ( vZ - vY,  vX, -vX)
                    if (!(ZEROd_EQ(vZ-vY) && ZEROd_EQ(vX))) {
                ...
                case VAM_POS_UP: {
                    // a = (0, 0, 1)        LEFT RIGHT 回転軸
                    // b = (vX, vY, vZ)
                    // a×b = (-vY, vX, 0)
                    if (!(ZEROd_EQ(vY) && ZEROd_EQ(vX))) {
...
void MyBunshinBase::addTurnAngleAroundAx(float prm_ax_x, float prm_ax_y, float prm_ax_z) {
    //現在未使用！
    //ある座標(x, y, z)において、回転の軸が v=(α, β, γ) (但し|v|=1) で、θ回す回転したい場合。
    //P = (0; x, y, z)
