...
public:
     ...
     */
    static GgafDx::FigureActor* activateMyShipItemOf(GgafDx::GeometricActor* prm_pActor);
    /**
...
public:
     ...
     * ・ショットの GgafDx::LocusVehicle の移動方向、移動速度、加速度<BR>
     * @param prm_x               発射元X座標
     * @param prm_y               発射元Y座標
     * @param prm_z               発射元Z座標
     * @param prm_rz              発射方向Rz
    ...
    /**
     * 対象アクターに紐ついた 共通的なエフェクト（アクターとSE） を取得できれば有効にし、それを返す .
     * @param prm_pActor 対象アクター
     * @param prm_status_kind エフェクト種類
     ...
     */
    static GgafDx::FigureActor* activateCommonEffectOf(GgafDx::GeometricActor* prm_pActor, int prm_status_kind);
    static GgafDx::FigureActor* activateMyShipEffectOf(GgafDx::GeometricActor* prm_pActor, int prm_status_kind);

    /**
     * 衝突したヒットパートを考慮して、衝突オブジェクトの座標を返す。
     ...
     * @param out_z
     */
    static void adjustHitCoord(GgafDx::GeometricActor* prm_pTargetAtor, coord& out_x, coord& out_y, coord& out_z);
