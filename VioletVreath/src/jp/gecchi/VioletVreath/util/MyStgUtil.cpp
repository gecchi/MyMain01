...
GgafDx::FigureActor* MyStgUtil::activateMyShipItemOf(GgafDx::GeometricActor* prm_pActor) {
    GgafDx::FigureActor* pI = nullptr;
    ...
    if (pTargetActor) {
        switch (pPrmActorStatus->get(STAT_ItemKind)) {
            ...
            case ITEM_MP_SMALL: {
                pI = MyShipScene_dispatch(MagicPointItem001);
                break;
            }
            case ITEM_MP_MIDDLE: {
                pI = MyShipScene_dispatch(MagicPointItem002);
                break;
            }
            case ITEM_MP_LARGE: {
                pI = MyShipScene_dispatch(MagicPointItem003);
                break;
            }
            default: {
...
void MyStgUtil::adjustHitCoord(GgafDx::GeometricActor* prm_pTargetAtor, coord& out_x, coord& out_y, coord& out_z) {
    out_x = prm_pTargetAtor->_x;
    out_y = prm_pTargetAtor->_y;
    out_z = prm_pTargetAtor->_z;
    if (prm_pTargetAtor->instanceOf(Obj_LaserChip)) {
        //ヒットパートレベルに爆発位置を補正
        GgafCore::Checker* pChecker = prm_pTargetAtor->getChecker()->_pNextExChecker;
        if (pChecker) {
...
GgafDx::FigureActor* MyStgUtil::activateMyShipEffectOf(GgafDx::GeometricActor* prm_pActor, int prm_status_kind) {
    GgafCore::Status* pPrmActorStatus = prm_pActor->getStatus();
    GgafDx::GeometricActor* pTargetActor =  prm_pActor;
...
/////////////
    ...
    switch (effect_kind) {
        ...
        case EF_MY_BONUS001: {
            //スコアが表示される消滅エフェクト
            SpriteLabelBonus001* pLabel = MyShipScene_dispatchForce(SpriteLabelBonus001);
        ...
        case EF_MY_BONUS_FORMATION: {
            //スコアが表示される消滅エフェクト
            SpriteLabelBonus002* pLabel = MyShipScene_dispatchForce(SpriteLabelBonus002);
            pLabel->onDispatched(pTargetActor); //初期設定が行われる
            std::string s = XTOS(pPrmActorStatus->get(STAT_AddDestroyScorePoint));
            pLabel->update(s.c_str()); //破壊時得点が表示される
            pRet = pLabel;
        ...
        case EF_MY_TURBO: {
            //ターボエフェクト
            EffectTurbo002* pE = MyShipScene_dispatch(EffectTurbo002);
            pRet = pE;
            break;
        }
        default: {
            //_TRACE_("MyStgUtil::activateMyShipEffectOf() 対応 Effect が定義されてないので無視。prm_pActor="<<prm_pActor->getName()<<"("<<prm_pActor<<") prm_status_kind="<<prm_status_kind<<" effect_kind="<< effect_kind);
            pRet = nullptr;
            break;
        }
    }
...
GgafDx::FigureActor* MyStgUtil::activateCommonEffectOf(GgafDx::GeometricActor* prm_pActor, int prm_status_kind) {
    GgafCore::Status* pPrmActorStatus = prm_pActor->getStatus();
    GgafDx::GeometricActor* pTargetActor =  prm_pActor;
...
/////////////
    ...
    switch (effect_kind) {
        ...
        case EF_EXPLOSION001: {
            ...
            MyStgUtil::adjustHitCoord(pTargetActor, out_x, out_y, out_z);
...
//////////////////
        ...
        default: {
            //_TRACE_("MyStgUtil::activateCommonEffectOf() 対応 Effect が定義されてないので無視。prm_pActor="<<prm_pActor->getName()<<"("<<prm_pActor<<") prm_status_kind="<<prm_status_kind<<" effect_kind="<< effect_kind);
            pRet = nullptr;
            break;
        }
    }
...
GgafCore::Status* MyStgUtil::resetMyShipStatus(GgafCore::Status* p) {
    ...
    if (!p->hasBeenReset()) {
        ...
        p->set(STAT_DestroyedEffectKind, EF_NOTHING);  //やられ特殊エフェクト種別(編隊は全滅時)
        p->set(STAT_ItemKind, ITEM_NOTHING);  //やられアイテム種別(編隊は全滅時)
        p->set(STAT_ProperEffect01Kind, EF_MY_TURBO);  //その他固有エフェクト０１
        p->set(STAT_AttackShotKind, SHOT_NOTHING);  //通常ショット種別
        p->set(STAT_RevengeShotKind, SHOT_NOTHING);  //打ち返し種別
        p->set(STAT_DepositoryKind, DEPO_NOTHING);  //デポジトリ種別
    }
...
GgafCore::Status* MyStgUtil::resetFormationEres001Status(GgafCore::Status* p) {
    ...
    if (!p->hasBeenReset()) {
        ...
        p->set(STAT_DamagedEffectKind, EF_NOTHING);  //ダメージエフェクト種別
        p->set(STAT_DamagedSeKind, SE_NOTHING);  //ダメージSE種別
        p->set(STAT_DestroyedEffectKind, EF_MY_BONUS_FORMATION);  //やられ特殊エフェクト種別(編隊は全滅時)
...
GgafCore::Status* MyStgUtil::resetEnemyRatislaviaEyeStatus(GgafCore::Status* p) {
    ...
    if (!p->hasBeenReset()) {
        ...
        p->set(STAT_DamagedEffectKind, EF_NOTHING);  //ダメージエフェクト種別
        p->set(STAT_DamagedSeKind, SE_ENEMY_DAMAGED_001);  //ダメージSE種別
        p->set(STAT_DestroyedEffectKind, EF_MY_BONUS001);  //やられ特殊エフェクト種別(編隊は全滅時)
