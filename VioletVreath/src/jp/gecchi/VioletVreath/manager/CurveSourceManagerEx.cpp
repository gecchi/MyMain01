#include "CurveSourceManagerEx.h"
...
CurveSourceManagerEx::CurveSourceManagerEx(const char* prm_manager_name) :
        CurveSourceManager(prm_manager_name) {
}

CurveSource* CurveSourceManagerEx::processCreateResource(const char* prm_idstr, void* prm_pConnector) {
    CurveSource* pResource = nullptr;
    ...
    if (strcmp("Spl_001", prm_idstr) == 0) {
        ...
        pResource = NEW CurveSource(p, 17, 0.5);//粒度 0.2
    }
    ...
    if (strcmp("Spl_00201_", prm_idstr) == 0) {
        ...
        pResource = NEW CurveSource(p, 5, 0.2); //粒度 0.2
    }
    ...
    if (strcmp("Spl_00202_", prm_idstr) == 0) {
        ...
        double p[][3] = { //           X  ,                          Y ,                         Z
           ...
           { MyShip::lim_x_infront_ * 1.5        , MyShip::lim_y_bottom_ * 0.2 ,                        0.0 },
           { MyShip::lim_x_infront_ * 2.5        , MyShip::lim_y_bottom_ * 0.5 ,  MyShip::lim_z_left_ * 0.3 },
    ...
    if (strcmp("Spl_HAN", prm_idstr) == 0) {
        ...
        pResource = NEW CurveSource(p, 21, 0.2); //粒度 0.2
    }
    ...
    if (strcmp("Spl_Allas01", prm_idstr) == 0) {
        ...
        for (int i = 0; i < 17; i++) {
            //X
            p[i][0] = p[i][0] * MyShip::lim_x_infront_;
            //Y
        ...
        pResource = NEW CurveSource(p, 17, 0.2); //粒度 0.2
    }
    if (pResource == nullptr) {
        pResource = CurveSourceManager::processCreateResource(prm_idstr, prm_pConnector);
    }
    return pResource;
}
