...
GgafActorDepository* DepositoryManager::processCreateResource(const char* prm_idstr, void* prm_pConnector) {
    ...
    if (strcmp("Shot001", prm_idstr) == 0) {
        ...
        pCOMMON_SCENE->bringSceneMediator()->addSubGroup(pResource); //最後にaddSubGroupすべし（さもないと直ぐに削除対象になる）
        ...
        pCOMMON_SCENE->bringSceneMediator()->addSubGroup(pResource);
        ...
        pCOMMON_SCENE->bringSceneMediator()->addSubGroup(pDepoEffect);
