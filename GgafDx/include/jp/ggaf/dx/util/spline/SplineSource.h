#ifndef GGAF_DX_SPLINESOURCE_H_
#define GGAF_DX_SPLINESOURCE_H_
    ...
    public:
        ...
        void init(double *sp, int spnum) {
            if (spnum >= MaxSplineSize) {
                throwCriticalException("SplineSource::BSpline::init() MaxSplineSizeを超えています。spnum="<<spnum);
            }
...
public:
     ...
     */
    SplineSource(double prm_paaBase[][3], int num, double prm_accuracy);
     ...
     */
    SplineSource(double prm_paaBase[][3], int num, double prm_accuracy, RotMat& prm_rotmat);
     ...
     * SplineSource() の引数なしコンストラクタでで生成した場合に、本メソッドを別途呼び出す必要があります。
     * @param prm_paaBase 制御点座標の配列
     *                    [][0] X座標
     *                    [][1] Y座標
     *                    [][2] Z座標
