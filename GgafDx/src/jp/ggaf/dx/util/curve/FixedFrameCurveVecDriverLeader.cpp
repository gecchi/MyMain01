...
//    if (ABS(_distance_to_begin) <= PX_C(1)) {
    if ( (ucoord)(_distance_to_begin + PX_C(1)) <= (ucoord)(PX_C(2)) ) {
        //始点への距離が無い、間引く。
        //_TRACE_("＜警告＞FixedFrameCurveVecDriverLeader::start("<<prm_option<<") _pActor_target="<<_pActor_target->getName()<<
        ...
        _TRACE_("＜警告＞FixedFrameCurveVecDriverLeader::restart("<<_option<<") _pActor_target="<<_pActor_target->getName()<<
