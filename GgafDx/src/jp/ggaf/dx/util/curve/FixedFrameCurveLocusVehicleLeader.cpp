...
FixedFrameCurveLocusVehicleLeader::FixedFrameCurveLocusVehicleLeader(CurveManufacture* prm_pManufacture, GgafDx::LocusVehicle* prm_pLocusVehicle_target) :
...
void FixedFrameCurveLocusVehicleLeader::restart() {
...
//    if (ABS(_distance_to_begin) <= PX_C(1)) {
    if ( (ucoord)(_distance_to_begin + PX_C(1)) <= (ucoord)(PX_C(2)) ) {
        //始点への距離が無い、間引く。
        //_TRACE_("【警告】FixedFrameCurveLocusVehicleLeader::start("<<prm_option<<") _pActor_target="<<_pActor_target->getName()<<
        ...
        _TRACE_("【警告】FixedFrameCurveLocusVehicleLeader::restart("<<_option<<") _pActor_target="<<_pActor_target->getName()<<
...
void FixedFrameCurveLocusVehicleLeader::behave() {
    if (_is_leading) {
        ...
        if (_prev_point_index != _point_index) {
            ...
            if (_point_index == 0) {
                ...
                pLocusVehicle_target->setMvVelo((velo)(_distance_to_begin / frame_of_segment));
            } else {
                ...
                if (calc_d*1.06 < actually_d) {
                    ...
                    pLocusVehicle_target->setMvVelo(_pFixedFrameSplManuf->_paSPMvVeloTo[_point_index] * 1.05) ;
                } if (calc_d*0.99 > actually_d) {
                    //速度補正：距離が予想より近い(0.99倍以内になっている)ので少しゆっくりする(0.95倍のスピードにする)
                    pLocusVehicle_target->setMvVelo(_pFixedFrameSplManuf->_paSPMvVeloTo[_point_index] * 0.95) ;
                } else {
                    pLocusVehicle_target->setMvVelo(_pFixedFrameSplManuf->_paSPMvVeloTo[_point_index]);
                }
            }
        }
        _leading_frames++;
    }
    //_TRACE_(_pActor_target->getBehaveingFrame()<<": "<<_leading_frames<<": _cnt_loop="<<_cnt_loop<<"  _point_index="<<_point_index<<" velo="<<_pActor_target->getLocusVehicle()->getMvVelo()<<" xyz="<<_pActor_target->_x<<","<<_pActor_target->_y<<","<<_pActor_target->_z);
}
FixedFrameCurveLocusVehicleLeader::~FixedFrameCurveLocusVehicleLeader() {
