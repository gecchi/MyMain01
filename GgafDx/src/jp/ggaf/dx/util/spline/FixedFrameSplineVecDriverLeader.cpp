...
FixedFrameSplineVecDriverLeader::FixedFrameSplineVecDriverLeader(SplineManufacture* prm_pManufacture, GgafDx::VecDriver* prm_pVecDriver_target) :
...
void FixedFrameSplineVecDriverLeader::restart() {
...
//    if (ABS(_distance_to_begin) <= PX_C(1)) {
    if ( (ucoord)(_distance_to_begin + PX_C(1)) <= (ucoord)(PX_C(2)) ) {
        //始点への距離が無い、間引く。
        //_TRACE_("＜警告＞FixedFrameSplineVecDriverLeader::start("<<prm_option<<") _pActor_target="<<_pActor_target->getName()<<
        ...
        _TRACE_("＜警告＞FixedFrameSplineVecDriverLeader::restart("<<_option<<") _pActor_target="<<_pActor_target->getName()<<
...
void FixedFrameSplineVecDriverLeader::behave() {
    if (_is_leading) {
    ...
    //_TRACE_(_pActor_target->getBehaveingFrame()<<": "<<_leading_frames<<": _cnt_loop="<<_cnt_loop<<"  _point_index="<<_point_index<<" velo="<<_pActor_target->callVecDriver()->getMvVelo()<<" xyz="<<_pActor_target->_x<<","<<_pActor_target->_y<<","<<_pActor_target->_z);
}
FixedFrameSplineVecDriverLeader::~FixedFrameSplineVecDriverLeader() {
