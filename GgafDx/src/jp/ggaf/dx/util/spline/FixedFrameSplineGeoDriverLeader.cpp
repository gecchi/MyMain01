...
FixedFrameSplineGeoDriverLeader::FixedFrameSplineGeoDriverLeader(
                                       SplineManufacture* prm_pManufacture,
                                       GgafDx::GeoDriver* prm_pGeoDriver_target) :
        SplineLeader(prm_pManufacture, prm_pGeoDriver_target->_pActor) {
    _pGeoDriver_target = prm_pGeoDriver_target;
...
void FixedFrameSplineGeoDriverLeader::restart() {
...
//    if (ABS(_distance_to_begin) <= PX_C(1)) {
    if ( (ucoord)(_distance_to_begin + PX_C(1)) <= (ucoord)(PX_C(2)) ) {
        //始点への距離が無い、間引く。
        //_TRACE_("＜警告＞FixedFrameSplineGeoDriverLeader::start("<<prm_option<<") _pActor_target="<<_pActor_target->getName()<<
        ...
        _TRACE_("＜警告＞FixedFrameSplineGeoDriverLeader::restart("<<_option<<") _pActor_target="<<_pActor_target->getName()<<
...
void FixedFrameSplineGeoDriverLeader::behave() {
    if (_is_leading) {
        GgafDx::GeoDriver* const pGeoDriver_target = _pGeoDriver_target;
        ...
        if ( _point_index == _pFixedFrameSplManuf->_pSpl->_rnum) {
            if (_cnt_loop == _max_loop) {
                //終了
                _is_leading = false;
                pGeoDriver_target->stopGravitationMvSequence();
                return;
            } else {
                //ループ
                _cnt_loop++;
