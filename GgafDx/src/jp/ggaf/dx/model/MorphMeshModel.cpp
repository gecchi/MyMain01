...
HRESULT MorphMeshModel::draw(FigureActor* prm_pActor_target, int prm_draw_set_num, void* prm_pPrm) {
    ...
    if (pModelLastDraw != this) {
        ...
        hr = pID3DXEffect->SetFloat(pMorphMeshEffect->_h_fog_starts_far_rate, _fog_starts_far_rate);
...
//            nFaceNormals = papMeshesFront[pattern]->_nFaceNormals;
            ...
            if (pattern == 0) { //プライマリメッシュ
                prepareVtx3D((void*)paVtxBuffer_data_primary, _size_vertex_unit_primary,
                ...
                prepareVtx3D((void*)(papaVtxBuffer_data_morph[pattern-1]), _size_vertex_unit_morph,
                           papModel3D[pattern], paNumVertices);
            }
