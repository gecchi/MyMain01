...
Puppeteer::Puppeteer(D3DXAniMeshActor* prm_pPuppet) : GgafCore::Object() {
    _pPuppet = prm_pPuppet;
    _pModel = (D3DXAniMeshModel*)_pPuppet->getModel();
    _num_perform = 0;
    _paPerformances = nullptr;
    _pAc = nullptr;
    //操り棒情報初期化
    //左手の操り棒
    _aStick[PuppeteerStick::LEFT_HAND]._no = 0;
    _aStick[PuppeteerStick::LEFT_HAND]._pPerformance = nullptr;
    //右手の操り棒
    _aStick[PuppeteerStick::RIGHT_HAND]._no = 1;
    _aStick[PuppeteerStick::RIGHT_HAND]._pPerformance = nullptr;
#ifdef MY_DEBUG
    if (!_pModel->_pAcBase) {
