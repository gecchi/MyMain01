...
Caretaker::Caretaker() : GgafCore::Caretaker() {
    ...
    if (CONFIG::FULL_SCREEN) {
        _primary_game_view_display_no = CONFIG::PRIMARY_GAME_VIEW_DISPLAY_NO;
        _secondary_game_view_display_no = CONFIG::SECONDARY_GAME_VIEW_DISPLAY_NO;
    } else {
        _primary_game_view_display_no = 0;
        _secondary_game_view_display_no = 1;
    }
    if (CONFIG::SWAP_VIEW && CONFIG::DUAL_VIEW) {
        int wk = _primary_game_view_display_no;
        _primary_game_view_display_no = _secondary_game_view_display_no;
        _secondary_game_view_display_no = wk;
    }
...
void Caretaker::setAppropriateDisplaySize(bool allow_chang_rezo) {
    ...
    if (CONFIG::FULL_SCREEN) {
        for (int adapter_no = 0; adapter_no < _num_adapter; adapter_no++) {
            ...
            if (CONFIG::DUAL_VIEW) {
                //２画面フルスクリーン時
                if (adapter_no == _primary_game_view_display_no) {
                    //ゲーム画面１画面目
                    ...
                    _paPresetPrm[_primary_game_view_display_no].BackBufferWidth  = CONFIG::PRIMARY_VIEW_FULL_SCREEN_WIDTH;
                    _paPresetPrm[_primary_game_view_display_no].BackBufferHeight = CONFIG::PRIMARY_VIEW_FULL_SCREEN_HEIGHT;
                } else if (adapter_no == _secondary_game_view_display_no) {
                    //ゲーム画面２画面目
                    ...
                    _paPresetPrm[_secondary_game_view_display_no].BackBufferWidth  = CONFIG::SECONDARY_VIEW_FULL_SCREEN_WIDTH;
                    _paPresetPrm[_secondary_game_view_display_no].BackBufferHeight = CONFIG::SECONDARY_VIEW_FULL_SCREEN_HEIGHT;
                } else {
...
#ifdef MY_DEBUG
    _TRACE_("初期設定");
    _TRACE_(" _primary_game_view_display_no = "<<_primary_game_view_display_no );
    _TRACE_(" _secondary_game_view_display_no = "<<_secondary_game_view_display_no );
    if (CONFIG::DUAL_VIEW) {
...
//        }
    ...
    if (CONFIG::FULL_SCREEN) {
        if (CONFIG::DUAL_VIEW) {
            if (_num_adapter < 2) {
                ...
                } else if(_primary_game_view_display_no == _secondary_game_view_display_no) {
    ...
    ZeroMemory(&_paPresetPrm[_primary_game_view_display_no], sizeof(D3DPRESENT_PARAMETERS));

    //ウィンドウ時・フルスクリーン時共通
    _paPresetPrm[_primary_game_view_display_no].BackBufferCount        = 1;            //バックバッファの数
    _paPresetPrm[_primary_game_view_display_no].EnableAutoDepthStencil = TRUE;         //バックバッファの Zバッファの自動作成
    _paPresetPrm[_primary_game_view_display_no].AutoDepthStencilFormat = D3DFMT_D24S8; //深度ステンシルバッファ //D3DFMT_D16;
    _paPresetPrm[_primary_game_view_display_no].Flags                  = 0;            //0にしておく

    if (CONFIG::FULL_SCREEN) {

        //フルスクリーン時
        _paPresetPrm[_primary_game_view_display_no].BackBufferFormat           = D3DFMT_X8R8G8B8; //D3DFMT_A8R8G8B8; //D3DFMT_X8R8G8B8; //D3DFMT_R5G6B5;
        _paPresetPrm[_primary_game_view_display_no].Windowed                   = false; //フルスクリーンモード時
        _paPresetPrm[_primary_game_view_display_no].FullScreen_RefreshRateInHz = D3DPRESENT_RATE_DEFAULT; //リフレッシュレート
        _paPresetPrm[_primary_game_view_display_no].PresentationInterval       = D3DPRESENT_INTERVAL_DEFAULT; //スワップのタイミング
        _paPresetPrm[_primary_game_view_display_no].SwapEffect                 = D3DSWAPEFFECT_DISCARD;

        if(CONFIG::DUAL_VIEW) {
            _paPresetPrm[_secondary_game_view_display_no] = _paPresetPrm[_primary_game_view_display_no]; //ココまでを複製

            _paPresetPrm[_primary_game_view_display_no].EnableAutoDepthStencil = FALSE;   //Zバッファの自動作成無効
            _paPresetPrm[_secondary_game_view_display_no].EnableAutoDepthStencil = FALSE;
            //【メモ】
            //EnableAutoDepthStencil = FALSE;
            //とすると、レンダリングステートの D3DRS_ZENABLE が FALSE になるだけのように見える。
            //従って「無効」にしただけであり、「使用不可」ではないのだろう・・・。
        } else {
            _paPresetPrm[_primary_game_view_display_no].EnableAutoDepthStencil = FALSE; //Z バッファの自動作成有効
        }
    } else {
        //ウィンドウ時
        _paPresetPrm[_primary_game_view_display_no].BackBufferFormat           = D3DFMT_UNKNOWN; //現在の表示モードフォーマット
        _paPresetPrm[_primary_game_view_display_no].Windowed                   = true; //ウィンドウモード時
        _paPresetPrm[_primary_game_view_display_no].FullScreen_RefreshRateInHz = 0; //リフレッシュレート
        _paPresetPrm[_primary_game_view_display_no].PresentationInterval       = D3DPRESENT_INTERVAL_IMMEDIATE; //即座
        _paPresetPrm[_primary_game_view_display_no].SwapEffect                 = D3DSWAPEFFECT_COPY; //TODO:Windowモードはこれ一択なのか？、D3DPRESENT_INTERVAL_ONE とかためす？

        if(CONFIG::DUAL_VIEW) {
            _paPresetPrm[_secondary_game_view_display_no] = _paPresetPrm[_primary_game_view_display_no]; //ココまでを複製
            _paPresetPrm[_primary_game_view_display_no].EnableAutoDepthStencil = TRUE; //Z バッファの自動作成
            _paPresetPrm[_secondary_game_view_display_no].EnableAutoDepthStencil = TRUE;
        } else {
            _paPresetPrm[_primary_game_view_display_no].EnableAutoDepthStencil = TRUE; //Z バッファの自動作成
        }
...
//        D3DADAPTER_DEFAULT,
//        D3DDEVTYPE_HAL,
//        _paPresetPrm[_primary_game_view_display_no].BackBufferFormat,  //TODO:ウィンドウモード時は
...
//            D3DADAPTER_DEFAULT,
//            D3DDEVTYPE_HAL,
//            _paPresetPrm[_primary_game_view_display_no].AutoDepthStencilFormat,
//            CONFIG::FULL_SCREEN ? FALSE : TRUE,
//            D3DMULTISAMPLE_2_SAMPLES,
//            nullptr) ) )
//        {
...
//    }
    ...
    if(CONFIG::DUAL_VIEW) {
        //マルチサンプルの数
        _paPresetPrm[_primary_game_view_display_no].MultiSampleType = multiSampleType;//D3DMULTISAMPLE_NONE;
        //マルチサンプルの品質レベル
        _paPresetPrm[_primary_game_view_display_no].MultiSampleQuality = qualityLevels - (qualityLevels > 0 ? 1 : 0);
        //マルチサンプルの数
        _paPresetPrm[_secondary_game_view_display_no].MultiSampleType = multiSampleType;//D3DMULTISAMPLE_NONE;
        //マルチサンプルの品質レベル
        _paPresetPrm[_secondary_game_view_display_no].MultiSampleQuality = qualityLevels - (qualityLevels > 0 ? 1 : 0);
    } else {
        //マルチサンプルの数
        _paPresetPrm[_primary_game_view_display_no].MultiSampleType = multiSampleType;//D3DMULTISAMPLE_NONE;
        //マルチサンプルの品質レベル
    ...
    //マルチサンプルの数
    //_paPresetPrm[_primary_game_view_display_no].MultiSampleType = D3DMULTISAMPLE_NONMASKABLE;//D3DMULTISAMPLE_NONE;
    //マルチサンプルの品質レベル
    //_paPresetPrm[_primary_game_view_display_no].MultiSampleQuality = 0;//qualityLevels - (qualityLevels > 0 ? 1 : 0);
    ...
    if (CONFIG::FULL_SCREEN) {
        if(CONFIG::DUAL_VIEW) {
            ...
            _paPresetPrm[_primary_game_view_display_no].BackBufferWidth  = CONFIG::PRIMARY_VIEW_FULL_SCREEN_WIDTH;
            _paPresetPrm[_primary_game_view_display_no].BackBufferHeight = CONFIG::PRIMARY_VIEW_FULL_SCREEN_HEIGHT;
            _paPresetPrm[_secondary_game_view_display_no].BackBufferWidth  = 0;
            _paPresetPrm[_secondary_game_view_display_no].BackBufferHeight = 0;
        }
    } else {
        if(CONFIG::DUAL_VIEW) {
            //ウィンドウモード・２画面使用
            _paPresetPrm[_primary_game_view_display_no].BackBufferWidth  = CONFIG::RENDER_TARGET_BUFFER_WIDTH;
            _paPresetPrm[_primary_game_view_display_no].BackBufferHeight = CONFIG::RENDER_TARGET_BUFFER_HEIGHT;
            _paPresetPrm[_secondary_game_view_display_no].BackBufferWidth  = 0;
            _paPresetPrm[_secondary_game_view_display_no].BackBufferHeight = 0;
        } else {
            //ウィンドウモード・１画面使用
    ...
    _paDisplayMode[_primary_game_view_display_no].Size = sizeof(_paDisplayMode[_primary_game_view_display_no]);
    _paDisplayMode[_primary_game_view_display_no].Width = _paPresetPrm[_primary_game_view_display_no].BackBufferWidth;
    _paDisplayMode[_primary_game_view_display_no].Height = _paPresetPrm[_primary_game_view_display_no].BackBufferHeight;
    _paDisplayMode[_primary_game_view_display_no].Format = _paPresetPrm[_primary_game_view_display_no].BackBufferFormat;
    _paDisplayMode[_primary_game_view_display_no].RefreshRate = _paPresetPrm[_primary_game_view_display_no].FullScreen_RefreshRateInHz;
    _paDisplayMode[_primary_game_view_display_no].ScanLineOrdering = D3DSCANLINEORDERING_PROGRESSIVE;

    _paDisplayMode[_secondary_game_view_display_no].Size = sizeof(_paDisplayMode[_secondary_game_view_display_no]);
    _paDisplayMode[_secondary_game_view_display_no].Width = _paPresetPrm[_secondary_game_view_display_no].BackBufferWidth;
    _paDisplayMode[_secondary_game_view_display_no].Height = _paPresetPrm[_secondary_game_view_display_no].BackBufferHeight;
    _paDisplayMode[_secondary_game_view_display_no].Format = _paPresetPrm[_secondary_game_view_display_no].BackBufferFormat;
    _paDisplayMode[_secondary_game_view_display_no].RefreshRate = _paPresetPrm[_secondary_game_view_display_no].FullScreen_RefreshRateInHz;
    _paDisplayMode[_secondary_game_view_display_no].ScanLineOrdering = D3DSCANLINEORDERING_PROGRESSIVE;

    //フルスクリーン時３画面目以降のパラメータを１画面のパラメータのコピーで作成
    for (int i = 0; i < _num_adapter; i++) {
        if (i != _primary_game_view_display_no && i != _secondary_game_view_display_no) {
            _paPresetPrm[i] = _paPresetPrm[_primary_game_view_display_no];
            _paDisplayMode[i] = _paDisplayMode[_primary_game_view_display_no];
        }
    }
    ...
    if (CONFIG::FULL_SCREEN) {
        if (CONFIG::DUAL_VIEW) {
            ...
            for (int n = 0; n < _num_adapter; n++) {
                if (n == _primary_game_view_display_no) {

                    RegisterClassEx(&prm_wndclass1);
                    _paHWnd[_primary_game_view_display_no] =
                            CreateWindowEx(
                                WS_EX_APPWINDOW,
                                ...
                                CW_USEDEFAULT,
                                CW_USEDEFAULT,
                                _paPresetPrm[_primary_game_view_display_no].BackBufferWidth,
                                _paPresetPrm[_primary_game_view_display_no].BackBufferHeight,
                                HWND_DESKTOP,
                                nullptr,
                                prm_wndclass1.hInstance,
                                nullptr
                            );
                } else if (n == _secondary_game_view_display_no) {
                    RegisterClassEx(&prm_wndclass2);
                    _paHWnd[_secondary_game_view_display_no] =
                            CreateWindowEx(
                                WS_EX_APPWINDOW,
                                ...
                                CW_USEDEFAULT,
                                CW_USEDEFAULT,
                                _paPresetPrm[_secondary_game_view_display_no].BackBufferWidth,
                                _paPresetPrm[_secondary_game_view_display_no].BackBufferHeight,
                                HWND_DESKTOP,
                                nullptr,
    ...
    _pHWndSecondary = _paHWnd[_secondary_game_view_display_no];
    _pHWndPrimary   = _paHWnd[_primary_game_view_display_no];
    //_pHWndPrimary の有無が
...
//#endif
    ...
    UINT adapter = _primary_game_view_display_no;
    if (CONFIG::FULL_SCREEN && CONFIG::DUAL_VIEW) {
        ...
        if (_vs_v < D3DVS_VERSION(3, 0) || _ps_v < D3DPS_VERSION(3, 0)) {
            ...
            hr = createDx9Device(adapter, D3DDEVTYPE_HAL, _pHWndPrimary,
                                 ...
                                 &_paPresetPrm[_primary_game_view_display_no], &_paDisplayMode[_primary_game_view_display_no]);
...
//                );
    ...
    hr = _pID3DDevice9->CreateDepthStencilSurface(
            CONFIG::RENDER_TARGET_BUFFER_WIDTH,
            CONFIG::RENDER_TARGET_BUFFER_HEIGHT,
            _paPresetPrm[_primary_game_view_display_no].AutoDepthStencilFormat, //D3DFORMAT           Format,
            _paPresetPrm[_primary_game_view_display_no].MultiSampleType,        //D3DMULTISAMPLE_TYPE MultiSample,
            _paPresetPrm[_primary_game_view_display_no].MultiSampleQuality,     //DWORD               MultisampleQuality,
    ...
    if (CONFIG::DUAL_VIEW) {
        ...
        for (int adapter = 0; adapter < _num_adapter; adapter++) {
            if (adapter == _primary_game_view_display_no) {
                hr = _pID3DDevice9->GetSwapChain(adapter, &_papSwapChain[_primary_game_view_display_no] );
                returnWhenFailed(hr, D3D_OK, "スワップチェイン取得に失敗しました(0)。adapter="<<adapter);
                hr = _papSwapChain[_primary_game_view_display_no]->GetBackBuffer( 0, D3DBACKBUFFER_TYPE_MONO, &_papBackBuffer[_primary_game_view_display_no] );
                returnWhenFailed(hr, D3D_OK, "スワップチェインから、ターゲットのバックバッファ取得に失敗しました(0)。adapter="<<adapter);
            } else if (adapter == _secondary_game_view_display_no) {
                hr = _pID3DDevice9->GetSwapChain( adapter, &_papSwapChain[_secondary_game_view_display_no] );
                returnWhenFailed(hr, D3D_OK, "スワップチェイン取得に失敗しました(1)。adapter="<<adapter);
                hr = _papSwapChain[_secondary_game_view_display_no]->GetBackBuffer( 0, D3DBACKBUFFER_TYPE_MONO, &_papBackBuffer[_secondary_game_view_display_no] );
        ...
        hr = _pID3DDevice9->GetSwapChain( 0, &_papSwapChain[_primary_game_view_display_no] );
    ...
    if (CONFIG::DUAL_VIEW) {
        hr = _pID3DDevice9->StretchRect(
                ...
                _papBackBuffer[_primary_game_view_display_no], &_aRect_FullScreen[PRIMARY_VIEW],
                D3DTEXF_NONE);
        checkDxException(hr, D3D_OK, "FULL_SCREEN DUAL_VIEW 1画面目、背景色塗に失敗しました。(1)\n"
                                     "_pRenderTextureSurface="<<_pRenderTextureSurface<<"/_papBackBuffer[_primary_game_view_display_no]="<<_papBackBuffer[_primary_game_view_display_no]);
        hr = _pID3DDevice9->StretchRect(
                _pRenderTextureSurface, &_aRect_HarfRenderBufferSource[SECONDARY_VIEW],
                _papBackBuffer[_secondary_game_view_display_no], &_aRect_FullScreen[SECONDARY_VIEW],
                D3DTEXF_NONE);
        checkDxException(hr, D3D_OK, "FULL_SCREEN DUAL_VIEW 2画面目、背景色塗に失敗しました。(1)\n"
                                     "_pRenderTextureSurface="<<_pRenderTextureSurface<<"/_papBackBuffer[_primary_game_view_display_no]="<<_papBackBuffer[_secondary_game_view_display_no]);
    } else {
        hr = _pID3DDevice9->StretchRect(
...
//    GGAF_RELEASE_BY_FROCE(_papBackBuffer[_primary_game_view_display_no]);
//    GGAF_RELEASE_BY_FROCE(_papSwapChain[_primary_game_view_display_no]);
//    if (CONFIG::DUAL_VIEW) {
//        GGAF_RELEASE_BY_FROCE(_papBackBuffer[_secondary_game_view_display_no]);
//        GGAF_RELEASE_BY_FROCE(_papSwapChain[_secondary_game_view_display_no]);
//    }
    return D3D_OK;
}
        ...
        if (CONFIG::FULL_SCREEN) {
            if (CONFIG::DUAL_VIEW) {
                ...
                hr = pDevice->StretchRect(
                        _pRenderTextureSurface, &_aRect_HarfRenderBufferSource[PRIMARY_VIEW],
                        _papBackBuffer[_primary_game_view_display_no], &_aRect_Present[PRIMARY_VIEW],
                        D3DTEXF_LINEAR); //TODO:D3DTEXF_LINEARをオプション指定にするか？
                //checkDxException(hr, D3D_OK, "FULL_SCREEN DUAL_VIEW 1画面目 StretchRect() に失敗しました。");
                hr = pDevice->StretchRect(
                        _pRenderTextureSurface, &_aRect_HarfRenderBufferSource[SECONDARY_VIEW],
                        _papBackBuffer[_secondary_game_view_display_no], &_aRect_Present[SECONDARY_VIEW],
                        D3DTEXF_LINEAR);
...
//            _TRACE_("【警告】デバイス異常発生!!" <<DXGetErrorString(hr) << " "<< DXGetErrorDescription(hr));
    ...
    if (pCARETAKER->_is_device_lost_flg) {
        ...
        while (true) {
            if (_can_wddm) {
                hr = ((IDirect3DDevice9Ex*)pDevice)->CheckDeviceState(_paPresetPrm[_primary_game_view_display_no].hDeviceWindow);
        ...
        if (CONFIG::FULL_SCREEN) {
            ...
            if (CONFIG::DUAL_VIEW) {
                SetWindowPos(_paHWnd[_primary_game_view_display_no], NULL, 0, 0,
                        _paPresetPrm[_primary_game_view_display_no].BackBufferWidth,
                        _paPresetPrm[_primary_game_view_display_no].BackBufferHeight,
                        SWP_NOMOVE | SWP_NOZORDER);
                SetWindowPos(_paHWnd[_secondary_game_view_display_no], NULL, 0, 0,
                        _paPresetPrm[_secondary_game_view_display_no].BackBufferWidth,
                        _paPresetPrm[_secondary_game_view_display_no].BackBufferHeight,
                        SWP_NOMOVE | SWP_NOZORDER);
            } else {
        ...
        for (int i = 0; i < 100*60*10; i++) {
            if (CONFIG::FULL_SCREEN && CONFIG::DUAL_VIEW) {
                ...
                hr = pDevice->Reset(&(_paPresetPrm[_primary_game_view_display_no]));
            }
            if (hr != D3D_OK) {
