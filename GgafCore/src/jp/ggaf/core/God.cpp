...
void* God::receive(uint64_t prm_cradle_no, Object* prm_pReceiver) {
    ...
    while (_is_loving_flg) {
        if (pCradle->_cradle_no == prm_cradle_no && (pCradle->_pReceiver == nullptr || pCradle->_pReceiver == prm_pReceiver) ) {
            while (_is_loving_flg) {
                if (pCradle->_phase < 2) {
                    _TRACE2_("＜受取人:"<<(prm_pReceiver ? prm_pReceiver->toString() : "nullptr")<<"("<<prm_pReceiver<<")＞ ねぇ神様、["<<prm_cradle_no<<"-"<<prm_pReceiver<<"]の祝福まだ～？、5ミリ秒だけ待ったげよう。pCradle->_phase="<<(pCradle->_phase));
#ifdef _DEBUG
                    //デバッグ時はタイムアウト無し
#else
                    ...
                    if (pCradle->_phase == 1) {
                        ...
                        _TRACE_N_("！");
                        continue; //待つ
                    } else if (pCradle->_phase == 0) {
                        //未着手？
                        pCradle = _pCradleRoot; //もう一度最初から探させる。
                        _TRACE_N_("…");
                        break;
                    } else if (pCradle->_phase == 2) {
                        //完成
                        pCradle = _pCradleRoot;
                        _TRACE_N_("(^_^)v");
                        break;
...
BEGIN_SYNCHRONIZED1; // ----->排他開始
            ...
            if (pCradleBlessing) {
                if (pCradleBlessing->_phase == 0) { //未着手ならまず作る
                    _TRACE2_("＜神＞ よし、ゆりかご["<<pCradleBlessing->_cradle_no<<"-"<<pCradleBlessing->_pReceiver<<"]は未着手(_phase == "<<pCradleBlessing->_phase<<")だな。ゆえに今から作ります！");
                    pCradleBlessing->_phase = 1; //ステータスを祝福中へ
...
END_SYNCHRONIZED1; // <----- 排他終了
                        ...
                        pCradleBlessing->_phase = 2; //ステータスを祝福済みへ
                    ...
                    _TRACE2_("＜神＞ ゆりかご["<<pCradleBlessing->_cradle_no<<"-"<<pCradleBlessing->_pReceiver<<"]の命は、もう祝福済みでストックしてるし・・・(_phase == "<<pCradleBlessing->_phase<<")。");
                }
            }
            if (_pCradleRoot == nullptr) {
                //無条件待機
...
END_SYNCHRONIZED1; // <----- 排他終了
                ...
                if (_pCradleRoot != nullptr && _pCradleRoot->_pCradle_prev->_phase == 0) {
