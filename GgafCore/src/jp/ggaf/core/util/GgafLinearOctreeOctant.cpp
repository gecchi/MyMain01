...
void GgafLinearOctreeOctant::registerElem(GgafLinearOctreeElem* const prm_pElem) {
    if (prm_pElem->_pOctant_current == this) {
        ...
        if (_pElem_first == nullptr) {
            ...
            //            _pNext = nullptr;
            //            _pPrev = nullptr;
            prm_pElem->_pOctant_current = this;
        } else {
            //末尾に追加の場合
    ...
    GgafLinearOctreeOctant* p = this; //= & _paLinearOctant[index]
    while (true) {
        if (p->_kindinfobit & this_kindbit) {
            //もう種別情報が設定済みならば、それ以上の親も設定済みの為、抜ける
            break;
        } else {
            //空間に種別情報が未設定ならば設定
            p->_kindinfobit |= this_kindbit;
        }
        if (index == 0) {
            break;
        }
