...
GgafXpm::GgafXpm(const char** prm_xpm) : GgafObject() {
    ...
    _char_on_pixel = 0;
    _c_px_non = '\0';
    _mapCharRgb['\0'] = NEW GgafRgb("#000000");
    ...
    for (int i = 0; i < _colors; i++) {
        ...
        if (str_color == "None" || str_color == "none" || str_color == "") {
            ...
            _mapCharRgb[c] = NEW GgafRgb(str_color);
        }
    }
    //pixels はヘッダー以降の配列
    ...
    for (int i = 0; i < _height; i++) {
        ...
        for (int j = 0; j < _width; j++) {
            if (pixels[i][j] == '\0') {
                throwGgafCriticalException("GgafXpm::GgafXpm() 縦横の数とデータが一致していません");
            } else {
