...
//                        GgafFactory::_pGarbageBox->add(pElementTemp->_pPrev); //ÉSÉ~î†Ç÷
//                    }
//                }
//            }
//        }
//
//        //
//        //_pDirectorà»â∫_last_frame_of_godÇÃçXêVÇÃÇ›çsÇ§ÅB
//
...
void GgafScene::behave() {
    ...
    if (_once_in_n_time == 1 || P_GOD->_frame_of_God % _once_in_n_time == 0) {
        ...
        callRecursive(&GgafElement<GgafScene>::behave); //çƒãA
    }
}
...
void GgafScene::judge() {
    ...
    if (_once_in_n_time == 1 || P_GOD->_frame_of_God % _once_in_n_time == 0) {
        ...
        callRecursive(&GgafElement<GgafScene>::judge); //çƒãA
    }
}
...
void GgafScene::doFinally() {
    if (_once_in_n_time == 1 || P_GOD->_frame_of_God % _once_in_n_time == 0) {
        ...
        callRecursive(&GgafElement<GgafScene>::doFinally); //çƒãA
    }
}
