...
void GgafGarbageBox::add(GgafActor* prm_pActor) {
    ...
    for (int i = 0; i < GgafGarbage_MAX_WAIT; i++) {
        if (GgafGarbageBox::_wait) {
            ...
            if (i == GgafGarbage_MAX_WAIT-1) {
                _DTRACE_("ÅÉåxçêÅÑ GgafGarbageBox::add("<<prm_pActor<<"["<<prm_pActor->getName()<<"]) ã≠êßadd");
            }
        } else {
            break;
        }
    ...
    _DTRACE_("ÉSÉ~î†(Actor) GgafGarbageBox::add("<<prm_pActor<<"["<<prm_pActor->getName()<<"])");
    GgafGarbageBox::_wait = false;
}
...
void GgafGarbageBox::add(GgafScene* prm_pScene) {
    ...
    for (int i = 0; i < GgafGarbage_MAX_WAIT; i++) {
        if (GgafGarbageBox::_wait) {
            Sleep(1);
            if (i == GgafGarbage_MAX_WAIT-1) {
                _DTRACE_("ÅÉåxçêÅÑ GgafGarbageBox::add("<<prm_pScene<<"["<<prm_pScene->getName()<<"]) ã≠êßadd");
            }
        } else {
            break;
        }
    ...
    if (prm_pScene->_pSceneDirector) { //äƒì¬Ç™Ç¢ÇÈèÍçáÇÕÅAäƒì¬ÇÕGgafGarbageBox::add(GgafActor* prm_pActor)Ç÷
        ...
        _DTRACE_("ÉSÉ~î†(Actor) GgafGarbageBox::add("<<prm_pScene<<"["<<prm_pScene->getName()<<"] ÇÃäƒì¬"<<pSceneDirector<<"["<<pSceneDirector->getName()<<"])");
    }
    _pDisusedScene->addSubFirst(prm_pScene->extract()); //addSubFirstÇ≈Ç∑ÅIaddSubLastÇ…îÒÇ∏
    _DTRACE_("ÉSÉ~î†(Scene) GgafGarbageBox::add("<<prm_pScene<<"["<<prm_pScene->getName()<<"])");
    GgafGarbageBox::_wait = false;
}
...
void GgafGarbageBox::clean(int prm_num_cleaning) {
    for (int i = 0; i < GgafGarbage_MAX_WAIT; i++) {
        if (GgafGarbageBox::_wait) {
            Sleep(1);
            if (i == GgafGarbage_MAX_WAIT-1) {
                _DTRACE_("ÅÉåxçêÅÑ GgafGarbageBox::clean() ã≠êßreturn");
                return;
            }
        } else {
            break;
