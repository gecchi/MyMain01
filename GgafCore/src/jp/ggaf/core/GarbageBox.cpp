...
void GarbageBox::add(Scene* prm_pScene) {
    ...
    if (prm_pScene->_pSceneMediator) { //íáâÓé“Ç™Ç¢ÇÈèÍçáÇÕÅAíáâÓé“ÇÕGarbageBox::add(Actor* prm_pActor)Ç÷
        ...
        _pDisusedActor->prependChild(pSceneMediator); //prependChildÇ≈Ç∑ÅIappendChildÇ…îÒÇ∏
        _TRACE_("ÉSÉ~î†(Actor) GarbageBox::add( "<<NODE_INFO_P(prm_pScene)<<"ÇÃíáâÓé“ÇÃ"<<NODE_INFO_P(pSceneMediator)<<")");
    }
