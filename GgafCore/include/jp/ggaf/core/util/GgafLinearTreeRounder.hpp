#ifndef GGAFCORE_GGAFLINEARTREEROUNDER_H_
#define GGAFCORE_GGAFLINEARTREEROUNDER_H_
    ...
    public:
        ...
        inline void push(T* prm_pObject) {
            if (_papCur == _papBanpei) {
                _TRACE_("＜警告＞ GgafLinearTreeRounder::push("<<prm_pObject<<") スタックを使い切りました。無視します。一箇所に当たり判定が塊過ぎです。");
                return;
            }
            (*_papCur) = prm_pObject;
            ++_papCur;
        ...
        inline void popush(TStack* prm_pTStack) {
            if (_papCur == _papBanpei) {
                _TRACE_("＜警告＞ GgafLinearTreeRounder::popush("<<prm_pTStack<<") スタックを使い切ってます。無視します。一箇所に当たり判定が塊過ぎです。");
                prm_pTStack->clear();
                return;
            }
            while ((*_papCur) = prm_pTStack->pop()) { //代入。pop出来なければ nullptr。 I know "=" , not "=="
                 ++_papCur;
                 if (_papCur == _papBanpei) {
                    _TRACE_("＜警告＞ GgafLinearTreeRounder::popush("<<prm_pTStack<<") スタックを使い切りました。無視します。一箇所に当たり判定が塊過ぎです。");
                    prm_pTStack->clear();
                    break;
                }
            }
...
//            }
    ...
    void executeRoundRobin(TStack* prm_pStackA, TStack* prm_pStackB) {
        //GgafLinearTreeRounderでは、要素の指す(GgafObject*)インスタンスは Tが前提
