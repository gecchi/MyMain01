...
private:
    ...
    static inline uint32_t getMortonOrderNumFromXYZindex( uint32_t x_space_index, uint32_t y_space_index, uint32_t z_space_index ) {
        return LinearOctree::separateEveryThirdBit(x_space_index)    |
               LinearOctree::separateEveryThirdBit(y_space_index)<<1 |
               LinearOctree::separateEveryThirdBit(z_space_index)<<2;
    }
