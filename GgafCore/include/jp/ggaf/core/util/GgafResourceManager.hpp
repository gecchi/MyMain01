...
GgafResourceConnection<T>* GgafResourceManager<T>::connect(const char* prm_idstr, void* prm_connector) {
    ...
    if (pObj == nullptr) {
        ...
        add(pObj);
        _TRACE3_("GgafResourceManager<T>::connect [" << _manager_name << "]" << prm_idstr << "は無いので、新規作成して保持に決定");
        _is_connecting_resource = false;
        return pObj;
    } else {
        //生成済みならそれを返す。接続カウンタを＋１
        pObj->_num_connection++;
        _TRACE3_("GgafResourceManager<T>::connect [" << _manager_name << "]" << prm_idstr << "はあるので接続カウント＋１." << pObj->_num_connection);
        _is_connecting_resource = false;
        return pObj;
    }
}
