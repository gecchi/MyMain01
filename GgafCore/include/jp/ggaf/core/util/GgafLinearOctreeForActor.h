...
public:
    ...
    public:
        ...
        inline void popush(CollisionStack* prm_pCollisionStack) {
            if (_papCur == _papBanpei) {
                _TRACE_("＜警告＞ GgafLinearOctreeForActor::popush("<<prm_pCollisionStack<<") スタックを使い切ってます。無視します。一箇所に当たり判定が塊過ぎです。");
            ...
            while ((*_papCur) = prm_pCollisionStack->pop()) { //代入。pop出来なければ nullptr。 I know "=" , not "=="
                 ++_papCur;
                 if (_papCur == _papBanpei) {
                    _TRACE_("＜警告＞ GgafLinearOctreeForActor::popush("<<prm_pCollisionStack<<") スタックを使い切りました。無視します。一箇所に当たり判定が塊過ぎです。");
                    prm_pCollisionStack->clear();
                    break;
                }
            }
...
public:
     ...
     */
    explicit GgafLinearOctreeForActor(int prm_level);
