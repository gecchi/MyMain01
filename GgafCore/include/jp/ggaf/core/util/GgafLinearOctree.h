 ...
 *
 *  ＜モートン順序 ＞
 *  Z字の順番で番号を振る手法事態を指す言葉(4分木では左上、右上、左下、右下で4個)。8分木ではZ字のZ字で番号をふる。
 *  ＜モートン順序通し番号、空間番号＞
 *  上記の各レベル空間に振られている分割空間の番号のこと
 *  上記図の例ではLv2空間のモートン順序通し番号は0～15 というように使うこととする。実際は８分木なので立体である。
 *  ＜モートン順序位置＞
 *  上記図の例で4分割した場合(8分木では8分割)の左上、右上、左下、右下を それぞれ 0 1 2 3 (8分木では 0～7)と呼ぶ場合がある、
 *  【例１】
