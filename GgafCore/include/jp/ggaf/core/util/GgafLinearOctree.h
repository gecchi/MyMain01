 ...
 * 立体図が描けないため、4分木の図を例に言葉の説明するが。本クラスの機能は立体の８分木です。
 * <pre><code>
 *  +-----------+
 *  |           |
 *  |           |
 ...
 * <BR>
 * <B>＜モートン順序＞</B><BR>
 * Z字のような順番で番号を振る手法事態を指す言葉(4分木では左上、右上、左下、右下で4個)とする。<BR>
 * 8分木ではZ字のZ字で番号をふる手法の事を指す。<BR>
 * <BR>
 * <B>＜モートン順序通し番号＞</B><BR>
 * 上記図の各Level空間に振られている分割空間の番号のことを指す事とする。<BR>
 * 上記図の例ではLv2空間のモートン順序通し番号は0～15 というように使う。<BR>
 * 実際は８分木なので立体で、Lv2空間モートン順序通し番号は0~15*15 になる。<BR>
 * <BR>
 * <B>＜空間番号＞</B><BR>
 * 上記図のように、各空間Lv毎にモートン順序で振られた番号を指す。<BR>
 * <BR>
 * <B>＜空間(配列)要素番号＞</B><BR>
 * 空間番号の線形配列のインデックスを指す。<BR>
 ...
 *
 *  <B>【例２】</B><BR>
 *  「Lv1の空間番号3の子空間のモートン順序位置2」と表現した場合は、
 ...
 *  +--+--+
 *  </code></pre>
 *  のことを指し、これのモートン順序位置2なので、Lv2空間モートン順序通し番号 14 を指してることとする。<BR>
 *  その他使用例<BR>
