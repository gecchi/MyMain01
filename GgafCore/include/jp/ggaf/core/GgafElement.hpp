 ...
 * </TR><TR>
 * <TD>「ノードクラス」</TD>
 * <TD>GgafElementクラスを直接・間接継承しているクラスを指します。</TD>
 * </TR><TR>
 * <TD>「上位」「下位」</TD>
...
template<class T>
class GgafElement : public GgafObject {
...
template<class T>
GgafElement<T>::GgafElement(string prm_name) : GgafObject() ,
...
_isLast(false)
{
	...
	_pPrev = (T*)this;
	_name = prm_name;
	TRACE("template<class T> GgafElement<T>::GgafElement("+_name+")");
	_class_name = "GgafElement<T>";
...
T* GgafElement<T>::tear() {
	if (_pParent != NULL) {
		...
		throw_GgafCriticalException("[GgafElement<"<<_class_name<<">::tear()] ＜警告＞ "<<getName()<<"は、何所にも所属していません");
	}
...
template<class T>
void GgafElement<T>::setParent(T* prm_pParent) {
	_pParent = prm_pParent;
}

template<class T>
T* GgafElement<T>::getNext() {
	return (T*)_pNext;
}

template<class T>
T* GgafElement<T>::getPrev() {
	return (T*)_pPrev;
}


template<class T>
T* GgafElement<T>::getParent() {
	if (_pParent == NULL) {
		throw_GgafCriticalException("[GgafElement<"<<_class_name<<">::getParent()] Error! 親ノードがありません。");
	}
	return (T*)_pParent;
}
...
template<class T>
T* GgafElement<T>::getParent(string prm_parent_name) {
	_pNodeTemp = (T*)this;
	while(true) {
		_pNodeTemp = _pNodeTemp->_pParent;
		if (_pNodeTemp == NULL) {
			throw_GgafCriticalException("[GgafElement<"<<_class_name<<">::getParent()] Error! 親ノードがありません。(prm_parent_name="+prm_parent_name+")");
...
template<class T>
T* GgafElement<T>::getSub(string prm_sub_actor_name) {
	if (_pSubFirst == NULL) {
		throw_GgafCriticalException("[GgafElement<"<<_class_name<<">::getSub()] Error! _pSubFirstがNULLです。");
	}
	...
	do {
		...
		if (_pNodeTemp -> _isLast) {
			throw_GgafCriticalException("[GgafElement<"<<_class_name<<">::getSub()] Error! 子ノードは存在しません。(prm_sub_actor_name="+prm_sub_actor_name+")");
		} else {
...
template<class T>
T* GgafElement<T>::getSubFirst() {
	return (T*)_pSubFirst;
}


template<class T>
bool GgafElement<T>::hasSub(string prm_sub_actor_name) {
	if (_pSubFirst == NULL) {
		return false;
	} else {
		_pNodeTemp = _pSubFirst;
...
template<class T>
void GgafElement<T>::addSubLast(T* prm_pSub) {
	if (prm_pSub->_pParent != NULL) {
		throw_GgafCriticalException("[GgafElement<"<<_class_name<<">::addSubLast()] Error! ノードは既に所属しています(this="<<_name<<"/prm_pSub="+prm_pSub->getName()+")");
	}
...
template<class T>
string GgafElement<T>::getName() {
	return _name;
}
/*
template<class T>
bool GgafElement<T>::isNamed(string prm_name) {
...
template<class T>
bool GgafElement<T>::isLast() {
	return _isLast;
}

template<class T>
bool GgafElement<T>::isFirst() {
	return _isFirst;
}

template<class T>
GgafElement<T>::~GgafElement() {
	_TRACE_("~GgafElement<"<<_class_name<<">() "+_name+" start-->");
	//自ノードのメンバの解放

//	if (_isAlive) {
//		_TRACE_("template<class T> GgafElement<T>::~GgafElement() "+_name+"：_isAliveはtrueおかしいクマー");
//	}
	...
	_TRACE_("~GgafElement<"<<_class_name<<">() "+_name+" <---end");
}
