 ...
 * <TR><TD>「Ｄ」</TD><TD>次(Next)ノード。上図では左から右が正順序としています。_pNext が指しているポインタです。</TD></TR>
 * <TR><TD>「Ｂ」</TD><TD>前(Prev)ノード。_pPrev が指しているポインタです。</TD></TR>
 * <TR><TD>「Ｐ」</TD><TD>親(Parent)ノード。自ノードが所属する連結リストの１つ上の階層のノードです。_pParentが指しているポインタです。全てのノードは直近の親ノードのポインタを持っています。</TD></TR>
 * <TR><TD>「Ｉ，Ｊ，Ｋ，Ｌ」</TD><TD>「Ｃ」子(Sub)ノードと呼びます。</TD></TR>
 * <TR><TD>「Ｉ」</TD><TD> 子ノードの中で特に「Ｉ」は子ノードの先頭ノードと呼び、自ノードの _pSubFirst が指しているポインタとなっています。</TD></TR>
 * </TABLE>
 * <BR>
 *
 * <B>【その他用語補足】</B>
...
//	virtual bool isNamed(string prm_name);
	 ...
	 * (Ｋ)⇔Ｉ⇔Ｊ⇔Ｋ⇔(Ｉ)
	 * -----------------------
	 *        ↓ 上図で自分が「Ｃ」とした場合、ここにノード「★」addSubLast すると
	 *        ↓ 次のような状態になる
	 * ----------------「実行後」
	 *      Ｃ
	 *      ↓
