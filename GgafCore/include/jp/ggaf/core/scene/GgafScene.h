 ...
 * また、シーンクラスは管理者(GgafLoadActor)を管理するクラスでもあります。 GgafLoadActor も参照して下さい。<BR>
 ...
 * ゲームというもの共通的に、様々な『シーン（場面）』で『アクター(キャラ)』たちが活躍するものだ、というイメージで設計してあります。<BR>
 * アプリ全体を仮にムービーが再生しているものと考えた場合、シーンオブジェクトとは、レイヤー（透明のセル）にあたるようなものという概念です。<BR>
 * <B>【用語補足】</B>
 * <TABLE border='1'>
 * <TR>
 * <TD>「シーン」</TD>
