...
public:
    ...
    /** [r/w] 共通遅延解放フレーム数。*/
    static frame END_DELAY_FRAME;
    /** [r/w] 最高フレームスキップ数。つまりどんなに重くも最低描画するフレーム間隔。*/
    static frame MAX_SKIP_FRAME;
...
//END_DELAY_FRAME について
//共通遅延解放フレーム数（≒エフェクト最大再生時間）
//END_DELAY_FRAMEは全Element共通の解放遅延フレーム数で、
...
//CONFIG::END_DELAY_FRAMEには、基本的に、画面から消滅確定→完全にdeleteまでの最小時間を指定する。
//現在は90秒分のフレーム。
//もし、CONFIG::END_DELAY_FRAMEを短めに設定すると、
