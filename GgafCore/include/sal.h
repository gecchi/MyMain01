...
//
//本ファイルはVC++2008 SDK の sal.h の コピーです。
//DirectX SDK August 2009 から"dsound.h"が"sal.h"を直接インクルードするようになった。
//SDKヘッダーが、非標準SDKヘッダーを更にincludeするなんて・・・・涙。
//sal.hは根幹に関わるヘッダーで、stddefのようなものだと思う。
//こまったことに、sal.h は VC用 Microsoft独自ヘッダであるのでVC++のSDKの入手が必要となる、
//よって DirectX SDK + MINGW だけではコンパイル不可になってしまった？！。
//しかし必要なファイルは sal.h たった一つだけで良いのじゃないか・・・？。と、疑心暗鬼にかかり、
//試行錯誤した結果GCCコンパイル時に、dsound.h が sal.h を発見できるように、sal.h のコピーをココに置く、
//ということでGCCでもコンパイル出来るようにになった。
//
//VC++2008でコンパイルする場合、sal.h は本ファイルが参照されるが同じなので問題無い。（コピーしただけなので）
//もちろん本ファイルを削除してもVC++2008 SDKのsal.hが参照されるので問題無い。
//VC++2009以降VC++2005以前でコンパイルする場合、本ファイル参照されると sal.h のバージョンが違うと
//エラーになってしまう可能性が高い。VisualStudio は includeパス優先度が、全体(オプション)のincludeパスよりも、
//プロジェクトのに設定されるincludeパスが優先されてしまい、これを回避する手段を考えるのが面倒である。
//よってVC++2008以外でコンパイルする場合は、本ファイルを削除しなければいけない。うーむカッコ悪い。
//実際VC++2005では本ファイルを削除しないとコンパイルエラーが発生。
//GCCでコンパイルする場合、本 sal.h をinclude すると NULL に __null が #define されてしまうが、
//困ったことにGCCで単にincludeした場合 __null は何も定義されない。
//本 sal.h で、NULL が正しく定義されるように、#define 等を定義を事前追加していくと、今度は "sourceannotations.h"
//というVC++SDKのヘッダファイルが必要になってしまう・・・。ということでもう諦めて、VCでコンパイルする場合以外は
//VC以外でのコンパイル時は NULL を 0 に強制的に再定義するようにした。
//
//TODO:これでいいのか！！ ＼(T_T)／
//
