...
//sal.hは根幹に関わるヘッダーで、GCCのstddefのようなものだと思う。
//こまったことに、sal.h はVC++のヘッダであるのでVCのSDKの入手が必要となる、
//よって DirectX SDK + GCC だけではコンパイル不可になってしまった。
//しかし必要なファイルは sal.h たった一つだけで良いのじゃないか・・・？と疑心暗鬼にかかり試行錯誤した結果
//GCCコンパイル時に、dsound.h が sal.h を発見できるように、sal.h のコピーを置く、という
//ことで一応はこんぱいるできるようになった。
//
//VC++2008でコンパイルする場合、sal.h は本ファイルが参照されるが同じなので問題ない。（コピーしただけなので）
//VC++2005でコンパイルする場合、VisualStudioはincludeパス優先度が、全体(オプション)のincludeパスよりも、
//プロジェクトのに設定されるincludeパスが優先されてしまうので、本ファイル参照されると sal.h のバージョンが
//違うためエラーになってしまう。よってVC++2005でコンパイルする場合は本ファイルを削除しておく。うーむカッコ悪い。
//GCCでコンパイルする場合、sal.h をinclude すると NULL に __null が #define されてしまうが、
//困ったことにGCCで単にincludeした場合 __null は何も定義されてない。
//sal.h で、NULL が正しく定義されるように、#define 等をいじって追加していくと、今度は "sourceannotations.h" という
//VC++SDKのヘッダファイルが必要になってしまう・・・。ということで諦めて、VCでコンパイルするとき以外は
//NULL を強制的に再定義するようにした。
//
//TODO:これでいいのか！！ ＼(T_T)／
//
