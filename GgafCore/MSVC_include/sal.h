...
//前提として、本プログラムはGCC(MINGWのGCC4.4)、VC++2008、VC++2005でコンパイル出来るような物を目指している。
//VC++2008、VC++2005では問題ないが、GCCでコンパイルすると時に本ファイルは必要となる。
//
//DirectX SDK August 2009 以降、なんとSDKのヘッダーコード内で直接VC用ヘッダファイル("sal.h"等)をincludeしているコードが現れた！。
//ヘッダーパスの優先順位を調整しても、どうにもこうにも回避できず、もはや DirectX SDK + MINGW のみでコンパイル出来なくなった（と思う）。
//しかし調べていく内に、「必要なVCのヘッダは sal.h たった一つだけでなんとかなるんじゃないか・・・？。」と、
//疑心暗鬼にかかり、試行錯誤の結果、sal.h のみのコピーだけで、GCCでもコンパイル出来るようにになった。
//１ファイルとは言え、依存性の高そうな回避はカッコ悪い。・・・時間がもったいないので、後で考えよう。
//
