...
public: //_X , _Y, _Z 操作関連 //////////////////////////////////////////////
    ...
    //「移動方角」の方向は、原点から単位球の表面に向かって伸びるベクトル (_vX, _vY, _vZ) で表す方法と、
    //緯度経度の２つのアングル値 (_angRzMove, _angRyMove) で表す方法の２種類用意した。
    //ここで緯度経度表現の場合、緯度＝Z軸回転角、経度＝Y軸回転角 と見立てる。２つのアングル値をセットで「移動方角」と呼ぶ場合もある。
    //ここでは、「移動方角（Y軸）」という言葉の語彙は、キャラの進行方向ベクトルのY軸回転角の成分（経度）を指していることとする。
    //詳しく表現すると「緯度経度表現での移動方角の、Y軸回転アングル値」と言う意味。
    //単に「移動方角」とした場合は、移動方向のベクトルを表現している（と思う)。
    //一方似た言葉で、「軸回転方角（Y軸）」と書いている場合は、キャラの向きのY軸回転角成分を指している。ぜんぜん別の意味である。
    //詳しく表現すると「キャラの向き表現の軸回転の内、Y軸回転角のアングル値」と言う意味。
    //「移動方角（X軸）」は存在しないが、「軸回転方角（X軸）」は当然存在する。混乱しないように。
    //緯度経度指定(_angRzMove, _angRyMove)と座標指定(_vX, _vY, _vZ) の内部保持メンバで、最終的にワールド変換で使用されるのは
    ...
    // ・移動方角（Z軸） → 軸回転方角(Z軸)
    // ・移動方角（Y軸） → 軸回転方角(Y軸)
    //しかし「軸回転方角」を設定ても「移動方角」変化しない（逆は関連しない）。
