 ...
 * 複数のオブジェクトを１回で描画し、最適化（高速化）を目論むことが出来る。 <BR>
 ...
 *  を超えると駄目である。<BR>
 * ・さらに、マテリアルは1種類としなければいけない。これは全体で1種類ではなく、<BR>
 *  各キャラごとにそれぞれマテリアル色が1種類しか割り当てれないという意味。しかし、テクスチャは全体で1種類しか駄目。<BR>
 *  （複数マテリアルのXファイルを指定してもエラーにはならないが、CommitChangesが発生するため<BR>
 *   オブジェクトそれぞれ描画していることになり、意味が無くなくなる）<BR>
 * @version 1.00
 * @since 2009/06/15
