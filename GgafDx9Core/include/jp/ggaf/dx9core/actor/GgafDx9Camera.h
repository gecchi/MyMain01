...
public:
     ...
     * このカメラの初期Z位置は、原点でDirctXの長さ1（のオブジェクト）を
     * 画面上10ピクセルで表示する位置に自動計算される。（_cameraZ_orgに保持)
     * この原点から、初期カメラZ位置までを1.0として、表示範囲の深さの倍数を引数で設定する。
     * prm_dep=1.0とした場合。表示範囲は 原点を中心とし、半径を初期カメラZ位置の距離の球とする。
     * また、表示範囲＋αが、アクター達のゲーム内での可動範囲となるため、カメラの初期設定値は
     * 非常に重要である。
