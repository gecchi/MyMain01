...
//! メモリ読み込み
size_t OggVorbisMemory::read(void* buffer, size_t size, size_t maxCount,
		void* stream) {
	if (buffer == 0) {
		return 0;
	}
...
bool OggVorbisMemory::createBuffer(const char* filePath) {
	...
	// コールバック登録
	ov_callbacks callbacks = { &OggVorbisMemory::read, &OggVorbisMemory::seek,
			&OggVorbisMemory::close, &OggVorbisMemory::tell };
