...
void GgafDx9Util::init() {
    if (_was_inited_flg) {
        ...
        // SLANT_ANG_0[0]      = 0
        // SLANT_ANG_0[1(.7)]Å` = 1000Å`
        // SLANT_ANG_0[3(.4)]Å` = 2000Å`
        // SLANT_ANG_0[5(.2)]  = 3000Å`
        // SLANT_ANG_0[6(.9)]  = 4000Å`
        // SLANT_ANG_0[8(.7)]  = 5000Å` Ç∆Ç¢Ç¡ÇΩãÔçáÇ…Ç»ÇÈÇÊÇ§Ç…í≤êÆ
        ...
        //ang=4500(45.0ìx)à»ç~ÇÕéüÇÃîzóÒÇ…é˚ÇﬂÇÈÅB
        //ç≈ëÂóvëfINDEXÇÕ0.9996Ç»ÇÃÇ≈ 1000 Ç∆ÇµÇÊÇ§ÅB
        ...
        for (int ang = 0; ang <= 4500; ang++) {
            ...
            for (int i = index_slant_prev+1, d = 1; i <= index_slant; i++, d++) {
                if (i > 10000) {
                    _TRACE_("ÅÉåxçêÅÑëzíËîÕàÕà»è„ÇÃåXÇ´îzóÒINDEXÇê›íËÅBÉÅÉÇÉäÇ™îjâÛÇ≥ÇÍÇ‹Ç∑ÅBSLANT_ANG_0["<<i<<"]<="<<(ang*10));
                }
...
//			_TRACE_("ang="<<ang<<" slant="<<slant<<" index_slant="<<index_slant<<" vx,vy="<<vx<<","<<vy);
        ...
        for (int i = index_slant_prev+1, d = 1; i <= 10000; i++, d++) {
            if (i > 10000) {
                _TRACE_("ÅÉåxçêÅÑëzíËîÕàÕà»è„ÇÃåXÇ´îzóÒINDEXÇê›íËÅBÉÅÉÇÉäÇ™îjâÛÇ≥ÇÍÇ‹Ç∑ÅBSLANT_ANG_0["<<i<<"]<="<<(45000));
            }
...
//            _TRACE_("ang="<<ang<<" slant="<<slant<<" index_slant="<<index_slant<<" vx,vy="<<vx<<","<<vy);
        ...
        //åXÇ´ 95.4895 Å` 572.957 ÇÃ äpìxÇãÅÇﬂîzóÒÇ…é˚ÇﬂÇÈÅB
        //óvëfî‘çÜÇÕÅAåXÇ´*10 - 954
        ...
        // SLANT_ANG_3[0]Å`114.5      = 894000
        // SLANT_ANG_3[1]      = 844000
        // SLANT_ANG_3[2]Å`[3] = 845000
        // SLANT_ANG_3[4]Å`[5] = 846000 Ç∆Ç¢Ç¡ÇΩãÔçá
        ...
        for (int s = 0; s <= 9018; s++) {
            _TRACE_("SLANT_ANG_0["<<s<<"]="<<SLANT_ANG_0[s]<<" åXÇ´"<<(s/10000.0)<<"=äpìx"<<(SLANT_ANG_0[s]/1000.0));
        }
        for (int s = 0; s <= 9018; s++) {
...
angle GgafDx9Util::getAngleFromSlant(float prm_slant) {
    if (prm_slant >= 0) {
        ...
        if (prm_slant < 114.589) {
            ...
            float u_slant = d_slant / 20.0; //20ìôï™
            for (int i = 0; i < 20; i++) {
