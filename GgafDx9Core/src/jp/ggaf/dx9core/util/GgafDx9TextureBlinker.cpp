...
GgafDx9TextureBlinker::GgafDx9TextureBlinker(GgafDx9GeometricActor* prm_pActor) :
    GgafObject() {
    _pActor = prm_pActor;
    ...
    for (int axis = 0; axis < 3; axis++) {
        ...
        _method[axis] = NOBLINK;
    }
}

void GgafDx9TextureBlinker::behave() {
    for (int axis = 0; axis < 3; axis++) {
        if (_method[axis] == NOBLINK) {
            continue;
        } else if (_method[axis] == TARGET_BLINK_LINER) {
        ...
        } if (_method[axis] == TARGET_BLINK_ACCELERATION) {
            _scale[axis] += _velo_scale[axis];
            if (_velo_scale[axis] > 0 && _target_scale[axis] <= _scale[axis]) {
                _scale[axis] = _target_scale[axis];
                _method[axis] = NOBLINK;
        ...
        } else if (_method[axis] == BEAT_BLINK_LINER) {
            _scale[axis] += _velo_scale[axis];
        ...
        } else if (_method[axis] == BEAT_BLINK_TRIANGLEWAVE) {
            _scale[axis] += _velo_scale[axis];
...
void GgafDx9TextureBlinker::intoTargetScaleLinerUntil(int prm_target_scale, DWORD prm_spend_frame) {
    for (int axis = 0; axis < 3; axis++) {
        intoTargetScaleLinerUntil(axis, prm_target_scale, prm_spend_frame);
    }
}

void GgafDx9TextureBlinker::intoTargetScaleLinerUntil(int prm_axis, int prm_target_scale, DWORD prm_spend_frame) {
    //_TRACE_("intoTargetScaleLinerUntil prm_axis="<<prm_axis<<"/prm_target_scale="<<prm_target_scale<<"/prm_spend_frame="<<prm_spend_frame);
    _method[prm_axis] = TARGET_BLINK_LINER;
...
void GgafDx9TextureBlinker::intoTargetScaleAcceStep(int prm_target_scale, int prm_velo_scale, int prm_acce_scale) {
    for (int axis = 0; axis < 3; axis++) {
        intoTargetScaleAcceStep(axis, prm_target_scale, prm_velo_scale, prm_acce_scale);
    }
}

void GgafDx9TextureBlinker::intoTargetScaleAcceStep(int prm_axis, int prm_target_scale, int prm_velo_scale, int prm_acce_scale) {
    _method[prm_axis] = TARGET_BLINK_ACCELERATION;
    _target_scale[prm_axis] = prm_target_scale;
    _velo_scale[prm_axis] = prm_velo_scale;
    _acce_scale[prm_axis] = prm_acce_scale;
}

void GgafDx9TextureBlinker::intoTargetScaleLinerStep(int prm_target_scale, int prm_velo_scale) {
    for (int axis = 0; axis < 3; axis++) {
        intoTargetScaleLinerStep(axis, prm_target_scale, prm_velo_scale);
    }
}

void GgafDx9TextureBlinker::intoTargetScaleLinerStep(int prm_axis, int prm_target_scale, int prm_velo_scale) {
    _method[prm_axis] = TARGET_BLINK_LINER;
    _target_scale[prm_axis] = prm_target_scale;
...
void GgafDx9TextureBlinker::loopLiner(int prm_axis, DWORD prm_beat_spend_frame, float prm_beat_num) {
    _method[prm_axis] = BEAT_BLINK_LINER;
...
void GgafDx9TextureBlinker::beat(int prm_axis, DWORD prm_beat_spend_frame, DWORD prm_attack_frame, DWORD prm_rest_frame, float prm_beat_num) {
    _method[prm_axis] = BEAT_BLINK_TRIANGLEWAVE;
    _one_way_cnt[prm_axis] = 0;
    _stop_one_way_num[prm_axis] = (int)(prm_beat_num*2.0f);
...
void GgafDx9TextureBlinker::stopImmediately(int prm_axis) {
    _method[prm_axis] = NOBLINK;
}
