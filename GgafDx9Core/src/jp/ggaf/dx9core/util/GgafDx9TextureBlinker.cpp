...
void GgafDx9TextureBlinker::behave() {
    if (_method == NOBLINK) {
        ...
        _fPowerBlink += _velo_fPowerBlink;
        if (_beat_begin_frame + _beat_attack_frame == GgafGod::_pGod->_godframe) { //アタック頂点時
            _fPowerBlink = _top_fPowerBlink;
            ...
            _fPowerBlink = _bottom_fPowerBlink;
            _velo_fPowerBlink = 0;
        } else if (_beat_begin_frame + _beat_spend_frame == GgafGod::_pGod->_godframe) { //ループ終了時
            _beat_begin_frame = GgafGod::_pGod->_godframe;
...
//いつかまた使うでしょう
//            _TRACE_("---実行前");
//            _TRACE_("GgafGod::_pGod->_godframe="<<GgafGod::_pGod->_godframe);
...
void GgafDx9TextureBlinker::intoTargetBlinkLinerUntil(float prm_target_fPowerBlink, DWORD prm_spend_frame) {
    ...
    if (_velo_fPowerBlink == 0.0f) {
        _velo_fPowerBlink = prm_target_fPowerBlink - _fPowerBlink;
    }
}
...
void GgafDx9TextureBlinker::loopLiner(DWORD prm_beat_spend_frame, float prm_beat_num) {
    ...
    if (_velo_fPowerBlink == 0.0f) {
        _velo_fPowerBlink = _top_fPowerBlink - _fPowerBlink;
    }
}
