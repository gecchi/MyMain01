...
// _angRzMv は Z軸回転角、 _angRyMv は Y軸回転角 を意味している。必ず Z軸回転 → Y軸回転の順番であると定義する。
// これは緯度と経度、（仰角とアジマス）の関係に対応させようとした設計です。しかし、Y軸回転角≡経度 であるが、Z軸回転角は
// Z軸回転角 → Y軸回転角の順番である場合に限り、緯度と意味が同じになるという制限がある。しかしここでは簡略して、
// 緯度経度の、「緯度」＝「Z軸回転角」、「経度」＝「Y軸回転角」 と表現することとする。
// 成分を個別に表現したい場合は、それぞれ「移動方角（Z軸）」「移動方角（Y軸）」と書くことにする。
// 本クラスで単に「移動方角」と書いてある場合は、2つのアングル値を指していることが多い。
// これらを踏まえて「移動方角（Z軸）」という言葉の語彙は、もしまわりくどく一気に説明すると、
// 「原点から単位球表面に伸びる方向ベクトルを、XYZ座標では無く球表面に緯度経度の座標を引いた２つの数値で表現するとして、
//  緯度＝Z軸回転角、経度＝Y軸回転角 とみなした（∵Z軸回転 → Y軸回転の順番）場合のZ軸回転角成分のみのアングル値」
// ということになる。
// 「Rz」という表現は「移動方角（Z軸）（但しZ軸回転 → Y軸回転の順番）」を意味している。
// 「Ry」という表現は「移動方角（Y軸）」を意味している。
// 「RzRy」という表現は「Z軸回転 → Y軸回転の順番の移動方角」を表している。
//
// 緯度＝Z軸回転角、経度＝Y軸回転角（但しZ軸回転 → Y軸回転の順番） のそれぞれの0度は、共通で方向ベクトル(1, 0, 0) の方向とする。
...
// よく使うのは前者の方である。本クラスのメソッドも _angRzMv と _angRyMv を操作するものがほとんどである。
...
//2010/02/19追記
// ※たまに「RyRz」という表現が存在するが、これは「Y軸回転 → Z軸回転の順番の移動方角」を表しているので注意。
