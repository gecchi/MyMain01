...
// 単に「移動方角」と呼ぶ場合は、一般的に移動方向のベクトルのことを指すが、ここでは、主に２つのアングル値をセットの事を「移動方角」
// と呼ぶ。
...
// 「前方を向いて真右向き」 は [軸回転方角(Z軸), 軸回転方角(Y軸)]=[0, 90度] or [180度,270度] とで表現できる。
// （※軸回転方角(Y軸)は左手系座標のためY軸の正方向を向いて反時計回り）
// 実は 「前方」 も [180度,180度]とも表現できるし、「真後ろ」 は [0度,180度] とも表現できる。
// どちらも向いている方向は同じ、但し姿勢は異なる。姿勢が異なるとまずいキャラは注意すること。
