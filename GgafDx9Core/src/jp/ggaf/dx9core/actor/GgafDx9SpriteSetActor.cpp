...
void GgafDx9SpriteSetActor::setUvFlipPtnRenge(int prm_top, int prm_bottom = 1) {
    if (prm_top < 0 || prm_bottom > (_pSpriteSetModel->_pattno_uvflip_Max)) {
        throwGgafCriticalException("GgafDx9SpriteSetActor::setUvFlipPtnRenge アニメーションパターン番号が範囲外です。引数("<<prm_top<<","<<prm_bottom<<")");
    } else {
