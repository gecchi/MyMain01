...
void GgafDx9UntransformedActor::processDrawPrior() {
    ...
    if (_isActiveFlg && !_wasHiddenFlg && _canLiveFlg) {
        if (getAlpha() < 1.0) {
            ...
            //_Z Ç™ ÉJÉÅÉâ Å` ÉJÉÅÉâ+2000,000 ÇÃä‘Ç≈Ç†ÇÍÇŒíiäKÉåÉìÉ_ÉäÉìÉOÇÇ∑ÇÈÇ±Ç∆Ç∆Ç∑ÇÈÅB
...
void GgafDx9UntransformedActor::getWorldTransformRxRzRyScMv(GgafDx9UntransformedActor* prm_pActor, D3DXMATRIX& out_matWorld) {
    //WORLDïœä∑
    //íPà çsóÒ Å~ Xé≤âÒì] Å~ Zé≤âÒì] Å~ Yé≤âÒì] Å~ ägëÂèkè¨ Å~ ïΩçsà⁄ìÆÅ@ÇÃïœä∑çsóÒÇçÏê¨ÅïÉfÉoÉCÉXÇ…ê›íË .
...
void GgafDx9UntransformedActor::getWorldTransformRzMv(GgafDx9UntransformedActor* prm_pActor, D3DXMATRIX& out_matWorld) {
    //WORLDïœä∑
    //íPà çsóÒ Å~ Zé≤âÒì] Å~ ïΩçsà⁄ìÆÅ@ÇÃïœä∑çsóÒÇçÏê¨ÅïÉfÉoÉCÉXÇ…ê›íË
    // |cosZ  , sinZ , 0  , 0  |
    // |-sinZ , cosZ , 0  , 0  |
    // |0     , 0    , 1  , 0  |
    // |dx    , dy	 , dz , 1  |
...
void GgafDx9UntransformedActor::getWorldTransformScRzMv(GgafDx9UntransformedActor* prm_pActor, D3DXMATRIX& out_matWorld) {
    ...
    //WORLDïœä∑
    //íPà çsóÒ Å~ ägëÂèkè¨ Å~ Zé≤âÒì] Å~ ïΩçsà⁄ìÆÅ@ÇÃïœä∑çsóÒÇçÏê¨ÅïÉfÉoÉCÉXÇ…ê›íË
    // |sx*cosZ , sx*sinZ , 0    , 0  |
    // |sy*-sinZ, sy*cosZ , 0    , 0  |
    // |0       , 0       , sz   , 0  |
    // |dx      , dy	  , dz   , 1  |
