...
//        }
        ...
        //ñ{óàÇÕÅ™ÇÃÇÊÇ§Ç…dynamic_castÇ∑ÇÈÇÃÇ™îƒópìIÇ©Ç¬à¿ëSÅBÇµÇ©ÇµÅAë¨ìxUPÇÃÇΩÇﬂÅidynamic_castÇè»Ç´ÇΩÇ¢Ç™ÇΩÇﬂÅjÇ…ÅA
        //GgafDx9GeometricActorÇ…åàÇﬂë≈ÇøÉLÉÉÉXÉgÇµÇƒÇ¢Ç‹Ç∑ÅBäÎåØÇ≈Ç∑ÅB
        //àÍâû_can_bump_flg ÇñYÇÍÇ∏Ç…falseÇ…Ç∑ÇÈÇ±Ç∆Ç…ÇÊÇ¡ÇƒÅAÇ±Ç±ÇÃà¯êîÇ…GgafDx9GeometricActorÇ…
        //ÉLÉÉÉXÉgÇ≈Ç´Ç»Ç¢É|ÉCÉìÉ^ÇÕóàÇ»Ç¢ÇÊÇ§ Ç»Ç¡ÇƒÇ¢Ç‹Ç∑ÅB
        //ñúÇ™àÍóàÇΩÇÁÅEÅEÅEÇªÇÃéûÇ…Ç‹ÇΩçlÇ¶ÇÈÅB
...
void GgafDx9GeometricActor::dump() {
    _TRACE_("\t\t\t\t\t\t\t\t"<<_class_name<<"("<<this<<")["<<getName()<<"]("<<_X<<","<<_Y<<","<<_Z<<")@"<<_frame_of_active<<","<<_can_bump_flg<<","<<_is_active_flg<<_was_paused_flg<<_can_live_flg<<","<<_is_active_flg_in_next_frame<<_was_paused_flg_in_next_frame<<_can_live_flg_in_next_frame<<","<<_will_activate_after_flg<<"("<<_frame_of_life_when_activation<<")");
...
void GgafDx9GeometricActor::dump(string prm_parent) {
    _TRACE_(prm_parent << _class_name<<"("<<this<<")["<<getName()<<"]("<<_X<<","<<_Y<<","<<_Z<<")@"<<_frame_of_active<<","<<_can_bump_flg<<","<<_is_active_flg<<_was_paused_flg<<_can_live_flg<<","<<_is_active_flg_in_next_frame<<_was_paused_flg_in_next_frame<<_can_live_flg_in_next_frame<<","<<_will_activate_after_flg<<"("<<_frame_of_life_when_activation<<")");
    GgafActor* pActor_tmp = _pSubFirst;
    if (_pSubFirst != NULL) {
        while (true) {
