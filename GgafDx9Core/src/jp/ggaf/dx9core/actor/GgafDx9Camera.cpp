...
GgafDx9Camera::GgafDx9Camera(const char* prm_name, float prm_rad_fovX, float prm_dep) : GgafDx9GeometricActor(prm_name, NULL) {
    ...
    //画面アスペクト比(w/h)
    _screen_aspect = (FLOAT)(1.0f * GGAFDX9_PROPERTY(GAME_SPACE_WIDTH) / GGAFDX9_PROPERTY(GAME_SPACE_HEIGHT));
    ...
    _cameraZ = -1.0f * ((GGAFDX9_PROPERTY(GAME_SPACE_HEIGHT) / PX_UNIT) / 2.0f) / _tan_half_fovY;
    ...
    /*
     ...
     GGAFDX9_PROPERTY(GAME_SPACE_HEIGHT),
     1.0f,
     GGAFDX9_PROPERTY(GAME_SPACE_HEIGHT)
     );
     */
    ...
    _pMover->setMvAng(0,0,0);
    _pMover->setMvVelo(0);
    _pMover->setRzMvAngVelo(0);
    _pMover->setRyMvAngVelo(0);
    _pMover->_mv_ang_rz_target_flg = true;
    _pMover->_mv_ang_ry_target_flg = true;
    setHitAble(false);
    ...
    _X_ScreenLeft   = (int)(-1 * GGAFDX9_PROPERTY(GAME_SPACE_WIDTH) * LEN_UNIT / 2);
    _X_ScreenRight  = (int)(GGAFDX9_PROPERTY(GAME_SPACE_WIDTH) * LEN_UNIT / 2);
    _Y_ScreenTop    = (int)(GGAFDX9_PROPERTY(GAME_SPACE_HEIGHT) * LEN_UNIT / 2);
    _Y_ScreenBottom = (int)(-1 * GGAFDX9_PROPERTY(GAME_SPACE_HEIGHT) * LEN_UNIT / 2);
}
...
void GgafDx9Camera::setViewPoint(GgafDx9GeometricActor* prm_pActor) {
    _pViewPoint->setCoordinateBy(prm_pActor);
}
