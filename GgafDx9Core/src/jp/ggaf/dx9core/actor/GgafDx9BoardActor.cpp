...
void GgafDx9BoardActor::processDrawMain() {
    ...
    potentialGgafDx9CriticalException(hr, S_OK, "GgafDx9BoardActor::processDrawMain SetTechnique("<<_technique<<") に失敗しました。");

    UINT numPass;
    hr = pID3DXEffect->Begin( &numPass, D3DXFX_DONOTSAVESTATE | D3DXFX_DONOTSAVESHADERSTATE );
    potentialGgafDx9CriticalException(hr, D3D_OK, "GgafDx9BoardActor::processDrawMain Begin() に失敗しました。");
    for (UINT pass = 0; pass < numPass; pass++) {
        hr = pID3DXEffect->BeginPass(pass);
        potentialGgafDx9CriticalException(hr, D3D_OK, "GgafDx9BoardActor::processDrawMain BeginPass(0) に失敗しました。");
        _pBoardModel->draw(this);
        hr = pID3DXEffect->EndPass();
        potentialGgafDx9CriticalException(hr, D3D_OK, "GgafDx9BoardActor::processDrawMain EndPass() に失敗しました。");
    }
    hr = pID3DXEffect->End();
    potentialGgafDx9CriticalException(hr, D3D_OK, "GgafDx9BoardActor::processDrawMain End() に失敗しました。");
}
