...
GgafDx9ModelManager::GgafDx9ModelManager(const char* prm_manager_name) :
    ...
    if (paChar_PointSpriteModelineTemplate == NULL) {
        throwGgafCriticalException("[GgafDx9ModelManager::GgafDx9ModelManager] ポイントスプライト情報読込みテンプレート\""<<GGAFDX9_PROPERTY(DIR_SPRITE_MODEL)<<"ggaf_pointspritemodel_define.x\" が開けません。");
    }
    ...
    if(hr != DXFILE_OK) {
        throwGgafCriticalException("[GgafDx9ModelManager::GgafDx9ModelManager] RegisterTemplatesに失敗しました。\""<<GGAFDX9_PROPERTY(DIR_SPRITE_MODEL)<<"ggaf_pointspritemodel_define.x\"を確認して下さい。");
    }
...
//「経緯メモ（グチ） 2009/07/05」
//D3DXLoadMeshFromX でXファイルを読み込むと思った通りの頂点数（頂点インデックス）になる保証が無い。増えたり減ったりする可能性がある。
//例えば三角形ポリゴンでの直方体の場合、頂点8個、頂点インデックス12（面が12個）、法線ベクトル6個、法線インデックス12、となるが、
//このようなXファイルを作って D3DXLoadMeshFromX で読み込むと、勝手に頂点数が増えてしまうことで悩んでいた。
//これは、Xファイルでは法線情報を面に紐つけて持たせれるのに対し、DIRECTX側では法線情報を各頂点に持たせるため、
//頂点が8個では本来は面単位であるハズの法線が表現できなくなってしまうためと予想される。
//だが親切にも D3DXLoadMeshFromX 内で ID3DXMesh 構築時、面と面の陰影を正しくつけるため（エッジをクッキリ見せるため）に法線が頂点より優先されて、
//頂点の隣接する面の数（法線数）分、頂点を増やす、という処理が勝手に行われるのであろう。立方体の場合は頂点が24個になってしまう。
//しかし、シェーダーでのモーフ時や、複数オブジェクトをDrawIndexedPrimitiveでわけて表示する時等、
//勝手に頂点の数が変わってしまうのは致命的であることが解るようになった。
//「Xファイルから、自分で決めた通りの頂点数で頂点バッファを作りたい。」と考え試行錯誤した。
//D3DXLoadMeshFromX で頂点が増減する最適化などの記事はあれど、その完全な仕様（アルゴリズム）は公開されていない（と思う）。
//D3DXLoadMeshFromX のソースコードさえあれば全て解決するのだが、これも公開されて無い。
//そんなわけで、内部で何やってるか90%ぐらいわかるのに、完全にはわからない・・・ならば D3DXLoadMeshFromX は使えない。という結論に行きついた。
//シェーダーを効果的に使うには独自にXファイルを読み込むしかないのかな、めんどくさいなぁ、でもしかたないのか。何かいい方法は？
//要は、「頂点」「法線」「マテリアル」「UV」「必要な他の情報」をファイルから読めればいいだろう、ならばなんでもいいじゃないか。
//Xファイルなんて、めんどくさいフォーマットじゃなくても・・・というかXファイルってむしろ何さ？！、という事がだんだん解ってきた。
//しかし D3DXLoadMeshFromX が使えないと結論付けるまでの期間が長かったお陰で、私はXファイルマスターになっていた。
//そこで、せっかくなのでXファイルからモデル情報を読む処理を独自に作ろうと決めたのだった。
//とりあえず、メタセコイア、3DSMAXのエクスポートするXファイルの静的モデルが読み込めれば良いかな、という気持ちで作成開始。
...
//しかしこのサンプルは、結構バグや未考慮な点が多くて大変だった。でも０から作るよりは遥かにマシで、勉強にもなったので良しとしよう。
//結果的に、頂点やマテリアルを独自で管理する事は、今から考えればいずれは必ず通る道だったろう。ただ、時間がかかり過ぎ。もっと時間が欲しい。
//このあたりの事って、他の人達はいったいどうやってるのだろうか？、みんな悩んでないのかなぁ。
//ゲームの企業だったなら、既にもっと高度で完全なライブラリが揃ってるんだろうな。
