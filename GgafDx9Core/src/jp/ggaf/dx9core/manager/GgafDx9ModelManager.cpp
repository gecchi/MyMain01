...
//「経緯メモ（グチ） 2009/07/05」
//シェーダーの勉強を始めた頃、D3DXLoadMeshFromX で読み込むと勝手に頂点バッファが増えてしまうことに気がついて悩んでいた。
...
//頂点が8個では本来は面単位であるハズの法線が表現できなくなってしまうためと予想できる。問題は「予想」を「仕様」に置きかえれないところだった。
...
//勝手に頂点の数が変わってしまっては困るということが解るようになった。「自分で決めた通りの頂点数で頂点バッファを作りたい。」と考え試行錯誤した。
//結論は、法線数＝頂点数とすればいいのであるが、そんな事気にしてモデリングやってられない。
//D3DXLoadMeshFromX で頂点が増減する最適化などの記事はある。しかし、知りたいのは内部のバッファ作成アルゴリズム「全て」である。
//D3DXLoadMeshFromX のソースコードさえあれば解決するのだが、当然公開されて無い。
//そんなわけで、内部で何やってるか完璧にはわからない。ならば D3DXLoadMeshFromX は使えないのか？。という結論に行きついた。
//つまりシェーダーを使うには独自にXファイルを読み込むしかないのかな、めんどくさいなぁ、でもしかたないのか。何かいい方法は？
...
//今から考えれば頂点情報を自前で管理する事は、いずれ必ず通る道だったろう。ただ、それ至るまでの時間がかかり過ぎだ。時間が全然足りない。
//無限の組み合わ数とも思えるトライアンドエラーで挫折してもオカシクナイ。（・・・しかし、これはまだ序章に過ぎなかった！。TODOだらけだし（笑））
