...
HRESULT GgafDx9MeshModel::draw(GgafDx9DrawableActor* prm_pActor_Target) {
    ...
    if (GgafDx9ModelManager::_pModelLastDraw != this) {
        ...
        checkDxException(hr, D3D_OK, "GgafDx9MeshModel::draw() SetFloat(_hPowerBlink) に失敗しました。");
        hr = pID3DXEffect->SetFloat(pMeshEffect->_hBlinkThreshold, _fBlinkThreshold);
        checkDxException(hr, D3D_OK, "GgafDx9MeshModel::draw() SetFloat(_hBlinkThreshold) に失敗しました。");
    }
    ...
    for (UINT i = 0; i < _nMaterialListGrp; i++) {
        ...
        if (GgafDx9ModelManager::_pModelLastDraw != this || _nMaterialListGrp != 1) {
            if (_papTextureCon[material_no] != NULL) {
                //テクスチャをs0レジスタにセット
                GgafDx9God::_pID3DDevice9->SetTexture(0, _papTextureCon[material_no]->refer()->_pIDirect3DTexture9);
            } else {
