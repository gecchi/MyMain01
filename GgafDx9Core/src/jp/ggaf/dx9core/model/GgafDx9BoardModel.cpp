...
HRESULT GgafDx9BoardModel::draw(GgafDx9BaseActor* prm_pActor_Target) {
    ...
    potentialGgafDx9CriticalException(hr, D3D_OK, "GgafDx9BoardModel::draw SetFloat(_hOffsetU) に失敗しました。");
    hr = pID3DXEffect->SetFloat(pBoardEffect->_hOffsetV, pRectUV_Active->_aUV[0].tv);
    potentialGgafDx9CriticalException(hr, D3D_OK, "GgafDx9BoardModel::draw SetFloat(_hOffsetV) に失敗しました。");

    hr = pID3DXEffect->SetFloat(pBoardEffect->_hTransformedX, pTargetActor->_x);
    potentialGgafDx9CriticalException(hr, D3D_OK, "GgafDx9BoardModel::draw SetFloat(_hTransformedX) に失敗しました。");
    hr = pID3DXEffect->SetFloat(pBoardEffect->_hTransformedY, pTargetActor->_y);
    potentialGgafDx9CriticalException(hr, D3D_OK, "GgafDx9BoardModel::draw SetFloat(_hTransformedY) に失敗しました。");
    hr = pID3DXEffect->SetFloat(pBoardEffect->_hDepthZ, pTargetActor->_z);
    potentialGgafDx9CriticalException(hr, D3D_OK, "GgafDx9BoardModel::draw SetFloat(_hDepthZ) に失敗しました。");
    //α設定
    hr = pID3DXEffect->SetFloat(pBoardEffect->_hAlpha, pTargetActor->_fAlpha);
    potentialGgafDx9CriticalException(hr, D3D_OK, "GgafDx9BoardModel::draw SetFloat(_hAlpha) に失敗しました。");
    hr = pID3DXEffect->CommitChanges();
    potentialGgafDx9CriticalException(hr, D3D_OK, "GgafDx9BoardModel::draw CommitChanges() に失敗しました。");
