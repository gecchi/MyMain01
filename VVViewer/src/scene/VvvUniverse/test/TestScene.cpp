...
void TestScene::processBehavior() {
    ...
    eye_face_no_ = DirectionUtil::cnvVec2FaceNo(eye_vx,eye_vy,eye_vz);

    if (eye_face_no_ != eye_face_no_prev_) {
        _TRACE_(" UP動く予感！");
        int eye_hx_prev, eye_hy_prev, eye_hz_prev;
        DirectionUtil::cnvFaceNo2Sgn(eye_face_no_prev_, eye_hx_prev, eye_hy_prev, eye_hz_prev);
        int eye_hx, eye_hy, eye_hz;
        DirectionUtil::cnvFaceNo2Sgn(eye_face_no_, eye_hx, eye_hy, eye_hz);
        ...
        DirectionUtil::cnvFaceNo2Sgn(_pVecUpActor->up_face_no_, up_hx_now, up_hy_now, up_hz_now);
        int up_face_no_next = DirectionUtil::_relation_up_by_vec[_pVecUpActor->up_face_no_][eye_face_no_];
        if (up_face_no_next != DirectionUtil::FACE_XXX) {
            if (up_face_no_next != DirectionUtil::FACE_ZZZ) {
                int up_hx_next, up_hy_next, up_hz_next;
                DirectionUtil::cnvFaceNo2Sgn(up_face_no_next, up_hx_next, up_hy_next, up_hz_next);
                ...
                int other_up_face_no_next = DirectionUtil::_relation_up_by_vec[_pVecUpActor->up_face_no_][eye_face_no_prev_];
                ...
                if (other_up_face_no_next != DirectionUtil::FACE_XXX) {
                    if (other_up_face_no_next != DirectionUtil::FACE_ZZZ) {
                        int up_hx_next, up_hy_next, up_hz_next;
                        DirectionUtil::cnvFaceNo2Sgn(other_up_face_no_next, up_hx_next, up_hy_next, up_hz_next);
