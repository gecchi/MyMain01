    ...
    for (UINT i = 0; i < _material_list_grp_num; i++) {
        ...
        if (GgafDxModelManager::_pModelLastDraw != this || _material_list_grp_num != 1) {
            if (_papTextureConnection[material_no]) {
                ...
                _TRACE_("GgafDxMeshModel::draw("<<prm_pActor_target->getName()<<") テクスチャがありません。"<<(CONFIG::WHITE_TEXTURE)<<"が設定されるべきです。おかしいです");
                //無ければテクスチャ無し
...
//        nFaceNormals = pMeshesFront->_nFaceNormals; //メッシュ連結後の総法線数
        ...
        GgafDxMeshModel::INDEXPARAM* paParam_tmp = NEW GgafDxMeshModel::INDEXPARAM[_nFaces]; //最高にマテリアルがバラバラだった場合nFaces必要
