...
HRESULT GgafDxMassSpriteModel::draw(GgafDxFigureActor* prm_pActor_target, int prm_draw_set_num, void* prm_pPrm) {
    ...
    //対象Actor
    const GgafDxMassSpriteActor* const pTargetActor = (GgafDxMassSpriteActor*)prm_pActor_target;
    //対象MassSpriteActorのエフェクトラッパ
    GgafDxMassSpriteEffect* const pMassSpriteEffect = (GgafDxMassSpriteEffect*)prm_pActor_target->getEffect();
    //対象エフェクト
...
void GgafDxMassSpriteModel::restore() {
    ...
    if (!_paVtxBuffer_data_model) {
        ...
        if (names.size() == 1) {
            ...
            xfile_name = GgafDxModelManager::getSpriteFileName(names[0], "sprx");
        } else if (names.size() == 2) {
            _set_num = STOI(names[0]);
            xfile_name = GgafDxModelManager::getSpriteFileName(names[1], "sprx");
        } else {
        ...
        _nVertices = 4;
        _nFaces = 2;
        _paVtxBuffer_data_model = NEW GgafDxMassSpriteModel::VERTEX_model[_nVertices];
        _size_vertex_unit_model = sizeof(GgafDxMassSpriteModel::VERTEX_model);
        _size_vertices_model = sizeof(GgafDxMassSpriteModel::VERTEX_model) * _nVertices;
