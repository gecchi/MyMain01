...
GgafDxMassBoardModel::GgafDxMassBoardModel(const char* prm_model_name) : GgafDxMassModel(prm_model_name) {
    _TRACE3_("GgafDxMassBoardModel::GgafDxMassBoardModel(" << _model_name << ")");
    ...
    //void GgafDxModelManager::restoreMassBoardModel(GgafDxMassBoardModel*)
    //で行うようにした。要参照。
}

void GgafDxMassBoardModel::createVertexModel(GgafDxMassModel::VertexModelInfo* out_info) {
    int element_num = 3;
...
HRESULT GgafDxMassBoardModel::draw(GgafDxFigureActor* prm_pActor_target, int prm_draw_set_num) {
    ...
    if (prm_draw_set_num > _set_num) {
        _TRACE_("GgafDxMassBoardModel::draw() "<<_model_name<<" の描画セット数オーバー。_set_num="<<_set_num<<" に対し、prm_draw_set_num="<<prm_draw_set_num<<"でした。");
    }
#endif
    ...
    checkDxException(hr, D3D_OK, "[GgafDxMassBoardModel::draw] 頂点バッファのロック取得に失敗 model="<<_model_name);
    ...
    checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::draw() SetStreamSourceFreq 0 に失敗しました。prm_draw_set_num="<<prm_draw_set_num);
    ...
    if (pModelLastDraw != this) {
        ...
        checkDxException(hr, D3D_OK, "GgafDxMassBoardActor::draw() SetStreamSourceFreq 1 に失敗しました。");
        //頂点バッファとインデックスバッファを設定
        hr = pDevice->SetVertexDeclaration(_pVertexDeclaration); //頂点フォーマット
        checkDxException(hr, D3D_OK, "GgafDxMassBoardActor::draw() SetVertexDeclaration に失敗しました。");
        hr = pDevice->SetStreamSource(0, _pVertexBuffer_model      , 0, _size_vertex_unit_model);
        checkDxException(hr, D3D_OK, "GgafDxMassBoardActor::draw() SetStreamSource 0 に失敗しました。");
        hr = pDevice->SetStreamSource(1, _pVertexBuffer_instacedata, 0, _size_vertex_unit_instacedata);
        checkDxException(hr, D3D_OK, "GgafDxMassBoardActor::draw() SetStreamSource 1 に失敗しました。");
        hr = pDevice->SetIndices(_pIndexBuffer);
        checkDxException(hr, D3D_OK, "GgafDxMassBoardActor::draw() SetIndices に失敗しました。");

        hr = pID3DXEffect->SetFloat(pMassBoardEffect->_h_tex_blink_power, _power_blink);
        checkDxException(hr, D3D_OK, "GgafDxMassBoardActor::draw() SetFloat(_h_tex_blink_power) に失敗しました。");
        hr = pID3DXEffect->SetFloat(pMassBoardEffect->_h_tex_blink_threshold, _blink_threshold);
        checkDxException(hr, D3D_OK, "GgafDxMassBoardActor::draw() SetFloat(_h_tex_blink_threshold) に失敗しました。");

        if (_papTextureConnection[0]) {
            pDevice->SetTexture(0, getDefaultTextureConnection()->peek()->_pIDirect3DBaseTexture9);
        } else {
            _TRACE_("GgafDxMassBoardActor::draw("<<prm_pActor_target->getName()<<") テクスチャがありません。"<<(PROPERTY::WHITE_TEXTURE)<<"が設定されるべきです。おかしいです");
            //無ければテクスチャ無し
    ...
    if (pEffect_active != pMassBoardEffect || GgafDxFigureActor::_hash_technique_last_draw != prm_pActor_target->_hash_technique)  {
        if (pEffect_active) {
           _TRACE4_("GgafDxMassBoardModel::draw() EndPass("<<pEffect_active->_pID3DXEffect<<"): /_pEffect_active="<<pEffect_active->_effect_name<<"("<<pEffect_active<<")");
            hr = pEffect_active->_pID3DXEffect->EndPass();
            checkDxException(hr, D3D_OK, "GgafDxMassBoardActor::draw() EndPass() に失敗しました。");
            hr = pEffect_active->_pID3DXEffect->End();
            checkDxException(hr, D3D_OK, "GgafDxMassBoardActor::draw() End() に失敗しました。");
#ifdef MY_DEBUG
        ...
        _TRACE4_("GgafDxMassBoardModel::draw() SetTechnique("<<pTargetActor->_technique<<"): /actor="<<pTargetActor->getName()<<"/model="<<_model_name<<" effect="<<pMassBoardEffect->_effect_name);
        hr = pID3DXEffect->SetTechnique(pTargetActor->_technique);
        checkDxException(hr, S_OK, "GgafDxMassBoardActor::draw() SetTechnique("<<pTargetActor->_technique<<") に失敗しました。");

        _TRACE4_("GgafDxMassBoardModel::draw() BeginPass("<<pID3DXEffect<<"): /actor="<<pTargetActor->getName()<<"/model="<<_model_name<<" effect="<<pMassBoardEffect->_effect_name<<"("<<pMassBoardEffect<<")");
        UINT numPass;
        hr = pID3DXEffect->Begin( &numPass, D3DXFX_DONOTSAVESTATE );
        checkDxException(hr, D3D_OK, "GgafDxMassBoardActor::draw() Begin() に失敗しました。");
        hr = pID3DXEffect->BeginPass(0);
        checkDxException(hr, D3D_OK, "GgafDxMassBoardActor::draw() BeginPass(0) に失敗しました。");
        ...
        checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::draw() CommitChanges() に失敗しました。");
    }
    ...
    if (_num_pass >= 2) { //２パス目以降が存在
        ...
        checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::draw() １パス目 EndPass() に失敗しました。");

        for (UINT pass = 1; pass < _num_pass; pass++) {
            hr = pID3DXEffect->BeginPass(pass);
            checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::draw() "<<pass+1<<"パス目 BeginPass("<<pass<<") に失敗しました。");
            hr = pDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,
                                               0,
                                               0,
                                               _nVertices,
                                               ...
                                               _nFaces);
            hr = pID3DXEffect->EndPass();
            checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::draw() "<<pass+1<<"パス目 EndPass() に失敗しました。");
        }

        hr = pID3DXEffect->BeginPass(0);
        checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::draw() １パス目 BeginPass(0) に失敗しました。");
    }
    //前回描画モデル保持
...
void GgafDxMassBoardModel::restore() {
    _TRACE3_("GgafDxMassBoardModel::restore() " << _model_name << " start");
    HRESULT hr;
    if (!_paVtxBuffer_data_model) {
        ...
        if (names.size() == 1) {
            _TRACE_("GgafDxMassBoardModel::restore() "<<_model_name<<" の最大同時描画オブジェクト数は、デフォルトの"<<GGAFDXMASS_MAX_INSTACE<<" が設定されました。");
            ...
            throwGgafCriticalException("GgafDxMassBoardModel::restore() "<<
                    "_model_name には \"xxxxxx\" or \"8/xxxxx\" 形式を指定してください。 \n"<<
                    "実際は、_model_name="<<_model_name<<" でした。");
        }
        if (_set_num < 1 || _set_num > GGAFDXMASS_MAX_INSTACE) {
            throwGgafCriticalException("GgafDxMassBoardModel::restore() "<<_model_name<<"の最大同時描画オブジェクト数が不正。範囲は 1～"<<GGAFDXMASS_MAX_INSTACE<<"セットです。_set_num="<<_set_num);
        }
        if (xfile_name == "") {
    ...
    if (_pVertexBuffer_model == nullptr) {
        hr = GgafDxGod::_pID3DDevice9->CreateVertexBuffer(
                ...
                nullptr);
        checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::restore() _pID3DDevice9->CreateVertexBuffer 失敗 model="<<(_model_name));
        //バッファへ作成済み頂点データを流し込む
        void *pDeviceMemory;
        hr = _pVertexBuffer_model->Lock(0, _size_vertices_model, (void**)&pDeviceMemory, 0);
        checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::restore() 頂点バッファのロック取得に失敗 model="<<_model_name);
        memcpy(pDeviceMemory, _paVtxBuffer_data_model, _size_vertices_model);
        hr = _pVertexBuffer_model->Unlock();
        checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::restore() 頂点バッファのアンロック取得に失敗 model="<<_model_name);
    }
    ...
    if (_pIndexBuffer == nullptr) {
        hr = GgafDxGod::_pID3DDevice9->CreateIndexBuffer(
                                ...
                                &(_pIndexBuffer),
                                nullptr);
        checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::restore() _pID3DDevice9->CreateIndexBuffer 失敗 model="<<_model_name);
        void* pDeviceMemory;
        hr = _pIndexBuffer->Lock(0, 0, (void**)&pDeviceMemory,0);
        checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::restore() インデックスバッファのロック取得に失敗 model="<<_model_name);
        memcpy(pDeviceMemory, _paIndexBuffer_data, sizeof(WORD)*_nFaces*3);
        hr = _pIndexBuffer->Unlock();
        checkDxException(hr, D3D_OK, "GgafDxMassBoardModel::restore() インデックスバッファのアンロック取得に失敗 model="<<_model_name);
    }
    //デバイスにテクスチャ作成
