...
GgafDxMassMeshEffect::GgafDxMassMeshEffect(const char* prm_effect_name) : GgafDxMassEffect(prm_effect_name) {
   ...
   HRESULT hr;
    ...
    checkDxException(hr, D3D_OK, "GgafDxMassMeshEffect::GgafDxMassMeshEffect SetValue(g_vecLightFrom_World) に失敗しました。");
    //Diffuse反射
    hr = _pID3DXEffect->SetValue("g_colLightDiffuse", &(GgafDxGod::_d3dlight9_default.Diffuse), sizeof(D3DCOLORVALUE));
    checkDxException(hr, D3D_OK, "GgafDxMassMeshEffect::GgafDxMassMeshEffect SetValue(g_colLightDiffuse) に失敗しました。");
    //Ambient反射
    hr = _pID3DXEffect->SetValue("g_colLightAmbient", &(GgafDxGod::_d3dlight9_default.Ambient), sizeof(D3DCOLORVALUE));
    checkDxException(hr, D3D_OK, "GgafDxMassMeshEffect::GgafDxMassMeshEffect SetValue(g_colLightAmbient) に失敗しました。");

    hr = _pID3DXEffect->SetFloat("g_zf", pCam->getZFar());
    checkDxException(hr, D3D_OK, "GgafDxMassMeshEffect::GgafDxSpriteEffect SetFloat(g_zf) に失敗しました。");
...
void GgafDxMassMeshEffect::setParamPerFrame() {
    ...
    checkDxException(hr, D3D_OK, "GgafDxMassMeshEffect::setParamPerFrame SetMatrix(_h_matView) に失敗しました。_effect_name="<<_effect_name);
    hr = _pID3DXEffect->SetValue(_h_posCam, pCam->getVecCamFromPoint(), sizeof(D3DXVECTOR3) );
    checkDxException(hr, D3D_OK, "GgafDxMassMeshEffect::setParamPerFrame SetValue(_h_posCam) に失敗しました。_effect_name="<<_effect_name);
}
