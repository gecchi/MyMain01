...
GgafDxBoardActor::GgafDxBoardActor(const char* prm_name,
    ...
    _width_px = (int)(_pBoardModel->_fSize_BoardModelWidthPx); //幅(px)、setScale()等により変化する！
    _height_px = (int)(_pBoardModel->_fSize_BoardModelHeightPx); //高さ(px)、setScale()等により変化する！
...
void GgafDxBoardActor::processDraw() {
    ...
    hr = pID3DXEffect->SetFloat(_pBoardEffect->_h_rot_center_X, float(C_PX(_X)));
    checkDxException(hr, D3D_OK, "GgafDxBoardModel::draw SetFloat(_h_rot_center_X) に失敗しました。3");

    hr = pID3DXEffect->SetFloat(_pBoardEffect->_h_rot_center_Y, float(C_PX(_Y)));
    checkDxException(hr, D3D_OK, "GgafDxBoardModel::draw SetFloat(_h_rot_center_Y) に失敗しました。3");
    ...
    hr = pID3DXEffect->SetFloat(_pBoardEffect->_hSx, r_sx);
    checkDxException(hr, D3D_OK, "GgafDxBoardModel::draw SetFloat(_sx) に失敗しました。");
    hr = pID3DXEffect->SetFloat(_pBoardEffect->_hSy, r_sy);
