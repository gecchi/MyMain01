...
void GgafDxStringSpriteActor::processDraw() {
    ...
    checkDxException(hr, D3D_OK, "GgafDxSpriteSetModel::draw SetFloat(_ah_alpha) ‚ÉŽ¸”s‚µ‚Ü‚µ‚½B");
    ...
    for (int pack = 0; pack < len_pack_num+(remainder_len == 0 ? 0 : 1); pack++) {
        ...
        for (int i = 0; i < _draw_set_num; i++) {
            ...
            checkDxException(hr, D3D_OK, "GgafDxSpriteModel::draw() SetFloat(_h_offset_u) ‚ÉŽ¸”s‚µ‚Ü‚µ‚½B");
