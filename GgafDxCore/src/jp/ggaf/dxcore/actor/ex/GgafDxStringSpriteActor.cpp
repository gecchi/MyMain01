...
GgafDxStringSpriteActor::GgafDxStringSpriteActor(const char* prm_name, const char* prm_model, GgafStatus* prm_pStat) :
    ...
    //デフォルトの１文字の幅(px)設定
    for (int i = 0; i < 256; i++) {
        _px_chr_width[i] = (int)(_pSpriteSetModel->_model_width_px);
        _px_row_width[i] = 0;
    }
...
void GgafDxStringSpriteActor::update(const char* prm_str) {
    ...
    for (int i = 0; i < len+1; i++) {
        ...
        if (prm_str[i] == '\n') {
            if (max_len_px < _px_row_width[_nn]) {
                max_len_px = _px_row_width[_nn];
            }
            nn++;
            _px_row_width[nn] = 0;
            continue;
...
void GgafDxStringSpriteActor::processDraw() {
    ...
    if (_align == ALIGN_LEFT || _align == ALIGN_CENTER) {
        int nnn = 0; // num of \n now
        int pos = 0;
        pixcoord dx = (_align == ALIGN_CENTER ? -_px_row_width[nnn] / 2 : 0) +
                        (_px_chr_width[_draw_string[pos]] / 2);
        pixcoord dx_tmp = dx;
        float u, v;
        int pattno = 0;
        int draw_set_cnt = 0;
        while (true) {
            if (_draw_string[pos] == (int)('\0')) {
                ...
                dx = (_align == ALIGN_CENTER ? -_px_row_width[nnn] / 2 : 0) +
                        (_px_chr_width[_draw_string[pos]] / 2);
                dx_tmp = dx;
                dy -= _chr_height_px;
        ...
        int pos = _len-1;
        pixcoord dx = +1*(_px_chr_width[(int)(_draw_string[pos])] / 2);
        pixcoord dx_tmp = dx;
        float u, v;
        int pattno = 0;
        int draw_set_cnt = 0;
        ...
        while (true) {
            if (pos == -1) {
                ...
                dx = +1*(_px_chr_width[(int)(_draw_string[pos])] / 2);
                dx_tmp = dx;
                dy += _chr_height_px;
            ...
            dx = dx_tmp - (w + _px_chr_width[_draw_string[pos]]);
            dx_tmp = dx + w;
            hr = pID3DXEffect->SetFloat(pSpriteSetEffect->_ah_x[draw_set_cnt], PX_DX(dx));
            checkDxException(hr, D3D_OK, "SetFloat(_ah_x) に失敗しました。");
            hr = pID3DXEffect->SetFloat(pSpriteSetEffect->_ah_y[draw_set_cnt], PX_DX(dy));
