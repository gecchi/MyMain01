...
//疑心暗鬼にかかり、sal.h のコピーをインクルードパスの通った場所に設置しコンパイルしていた。
//そのうち、sal.h の #define 定義は、GCCでは全部空実装で問題ないじゃないのか？という身勝手な疑心暗鬼にまたもかかってしまった。
...
//#define NULL __null
//
//となっており、__null は埋め込み定数のようであり、64bit環境ではサイズは自動的に8バイトになる、そんな定数だ。
//__null は GCC 独特のマジック定数？だと思っていのが、sal.h になんと __null が #define 定義されているのである。え～って感じ。
...
//という対処を行なっている。
//GCCで #undef __null と定義を解除しても NULL が空にはならない。たぶん __null は埋め込み定数だからだ！
