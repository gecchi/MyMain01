...
public:
    ...
    static int calMyStamina(GgafCore::GgafActor* pMy, GgafCore::GgafActor* pEnemy) {
        ...
        if (domi > 0) {
            ...
            //自分が劣性時
            return pMy->_pStatus->minus(STAT_Stamina, pEnemy->_pStatus->get(STAT_AttackRecessive)); //相手の優性時の攻撃力を受ける
        } else {
            //相手と同格時
    ...
    static int calEnemyStamina(GgafCore::GgafActor* pEnemy, GgafCore::GgafActor* pMy) {
        ...
        if (domi > 0) {
            ...
            //敵が劣性時
            return pEnemy->_pStatus->minus(STAT_Stamina, pMy->_pStatus->get(STAT_AttackRecessive)); //自機の優性時の攻撃力を受ける
        } else {
            //相手と同格時
