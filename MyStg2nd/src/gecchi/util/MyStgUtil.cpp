...
// ÉRÅ[ÉhïœçXÇÕÅuÉVÅ[ÉìCreater.xlsÅvÇ©ÇÁçsÇ¡ÇƒÇ¢ÇΩÇæÇ´ÇΩÇ¢ÅB
// gen02 start
	void MyStgUtil::resetMyStraightLaserChip001Status(GgafActor* p) {
		p->_pStatus->set(STAT_DEFAULT_ACTOR_KIND, KIND_MY_SHOT_NOMAL);  //éÌï 
		p->_pStatus->set(STAT_AddScorePoint, 0 );  //â¡éZìæì_
		p->_pStatus->set(STAT_AddRankPoint, 0.00000 );  //â¡éZÉâÉìÉN
		p->_pStatus->set(STAT_Stamina, 100 );  //ëÃóÕ
		p->_pStatus->set(STAT_Attack, 100 );  //çUåÇóÕ
		p->_pStatus->set(STAT_AttackDominant, 200 );  //é©ï™Ç™óDê´éûÇÃçUåÇóÕ
		p->_pStatus->set(STAT_AttackRecessive, 50 );  //é©ï™Ç™óÚê´éûÇÃçUåÇóÕ
	}
	void MyStgUtil::resetMyShipStatus(GgafActor* p) {
		p->_pStatus->set(STAT_DEFAULT_ACTOR_KIND, KIND_MY_BODY);  //éÌï 
		p->_pStatus->set(STAT_AddScorePoint, 0 );  //â¡éZìæì_
		p->_pStatus->set(STAT_AddRankPoint, 0.00000 );  //â¡éZÉâÉìÉN
		p->_pStatus->set(STAT_Stamina, 100 );  //ëÃóÕ
		p->_pStatus->set(STAT_Attack, 100 );  //çUåÇóÕ
	...
	void MyStgUtil::resetEnemyCeresStatus(GgafActor* p) {
		p->_pStatus->set(STAT_DEFAULT_ACTOR_KIND, KIND_ENEMY_BODY);  //éÌï 
		p->_pStatus->set(STAT_AddScorePoint, 100 );  //â¡éZìæì_
		p->_pStatus->set(STAT_AddRankPoint, 0.10000 );  //â¡éZÉâÉìÉN
		p->_pStatus->set(STAT_Stamina, 100 );  //ëÃóÕ
		p->_pStatus->set(STAT_Attack, 100 );  //çUåÇóÕ
		p->_pStatus->set(STAT_AttackDominant, 200 );  //é©ï™Ç™óDê´éûÇÃçUåÇóÕ
		p->_pStatus->set(STAT_AttackRecessive, 50 );  //é©ï™Ç™óÚê´éûÇÃçUåÇóÕ
	...
	void MyStgUtil::resetEnemyCeresShot001Status(GgafActor* p) {
		p->_pStatus->set(STAT_DEFAULT_ACTOR_KIND, KIND_ENEMY_SHOT_GU);  //éÌï 
		p->_pStatus->set(STAT_AddScorePoint, 1 );  //â¡éZìæì_
		p->_pStatus->set(STAT_AddRankPoint, 0.00000 );  //â¡éZÉâÉìÉN
		p->_pStatus->set(STAT_Stamina, 100 );  //ëÃóÕ
		p->_pStatus->set(STAT_Attack, 100 );  //çUåÇóÕ
		p->_pStatus->set(STAT_AttackDominant, 200 );  //é©ï™Ç™óDê´éûÇÃçUåÇóÕ
	...
	void MyStgUtil::resetEnemyAstraeaStatus(GgafActor* p) {
		p->_pStatus->set(STAT_DEFAULT_ACTOR_KIND, KIND_ENEMY_BODY);  //éÌï 
		p->_pStatus->set(STAT_AddScorePoint, 300 );  //â¡éZìæì_
		p->_pStatus->set(STAT_AddRankPoint, 0.20000 );  //â¡éZÉâÉìÉN
		p->_pStatus->set(STAT_Stamina, 100 );  //ëÃóÕ
		p->_pStatus->set(STAT_Attack, 100 );  //çUåÇóÕ
		p->_pStatus->set(STAT_AttackDominant, 200 );  //é©ï™Ç™óDê´éûÇÃçUåÇóÕ
		p->_pStatus->set(STAT_AttackRecessive, 50 );  //é©ï™Ç™óÚê´éûÇÃçUåÇóÕ
	...
	void MyStgUtil::resetEnemyAstraeaLaserChip001Status(GgafActor* p) {
		p->_pStatus->set(STAT_DEFAULT_ACTOR_KIND, KIND_ENEMY_SHOT_NOMAL);  //éÌï 
		p->_pStatus->set(STAT_AddScorePoint, 0 );  //â¡éZìæì_
		p->_pStatus->set(STAT_AddRankPoint, 0.00000 );  //â¡éZÉâÉìÉN
		p->_pStatus->set(STAT_Stamina, 100 );  //ëÃóÕ
		p->_pStatus->set(STAT_Attack, 100 );  //çUåÇóÕ
	...
	void MyStgUtil::resetEnemyVestaStatus(GgafActor* p) {
		p->_pStatus->set(STAT_DEFAULT_ACTOR_KIND, KIND_ENEMY_BODY);  //éÌï 
		p->_pStatus->set(STAT_AddScorePoint, 1000 );  //â¡éZìæì_
		p->_pStatus->set(STAT_AddRankPoint, 0.50000 );  //â¡éZÉâÉìÉN
		p->_pStatus->set(STAT_Stamina, 100 );  //ëÃóÕ
		p->_pStatus->set(STAT_Attack, 100 );  //çUåÇóÕ
		p->_pStatus->set(STAT_AttackDominant, 200 );  //é©ï™Ç™óDê´éûÇÃçUåÇóÕ
		p->_pStatus->set(STAT_AttackRecessive, 50 );  //é©ï™Ç™óÚê´éûÇÃçUåÇóÕ
	...
	void MyStgUtil::resetEnemyIrisStatus(GgafActor* p) {
		p->_pStatus->set(STAT_DEFAULT_ACTOR_KIND, KIND_ENEMY_BODY);  //éÌï 
		p->_pStatus->set(STAT_AddScorePoint, 20 );  //â¡éZìæì_
		p->_pStatus->set(STAT_AddRankPoint, 0.20000 );  //â¡éZÉâÉìÉN
		p->_pStatus->set(STAT_Stamina, 100 );  //ëÃóÕ
		p->_pStatus->set(STAT_Attack, 100 );  //çUåÇóÕ
		p->_pStatus->set(STAT_AttackDominant, 200 );  //é©ï™Ç™óDê´éûÇÃçUåÇóÕ
