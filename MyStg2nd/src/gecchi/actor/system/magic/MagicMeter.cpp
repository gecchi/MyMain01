...
: GgafDx9BoardSetActor(prm_name, prm_model, "MagicMeterEffect", "MagicMeterTechnique") {
    ...
    _x = 100;
    _h_active_magic = _pID3DXEffect->GetParameterByName( NULL, "_g_active_magic" ); //É}ÉXÉ^Å[Éø
...
void MagicMeter::processDraw() {
    _draw_set_num = _ringMagics.length(); //ìØàÍï`âÊê[ìxÇ…ÅAGgafDx9BoardSetActorÇÃìØÇ∂ÉÇÉfÉãÇ©Ç¬ìØÇ∂ÉeÉNÉjÉbÉNÇ™
    ...
    for (int i = 0; i < _draw_set_num; i++) {
        ...
        if (pMagic != _ringMagics.getCurrent()) {
            ...
            checkDxException(hr, D3D_OK, "GgafDx9BoardSetModel::draw SetInt(_h_active_magic) Ç…é∏îsÇµÇ‹ÇµÇΩÅB");
        }
        hr = pID3DXEffect->SetInt(_h_magic_no, i);
        checkDxException(hr, D3D_OK, "GgafDx9BoardSetModel::draw SetInt(_h_magic_no) Ç…é∏îsÇµÇ‹ÇµÇΩÅB");
        pElem = pElem->_pNext;
    }
