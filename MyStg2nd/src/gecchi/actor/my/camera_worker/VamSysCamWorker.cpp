...
VamSysCamWorker::VamSysCamWorker(const char* prm_name) : CameraWorker(prm_name) {
    ...
    _lim_CAM_top     = MyShip::_lim_top     - (PROPERTY(GAME_BUFFER_HEIGHT)*LEN_UNIT/2)*revise;
    _lim_CAM_bottom  = MyShip::_lim_bottom  + (PROPERTY(GAME_BUFFER_HEIGHT)*LEN_UNIT/2)*revise;
    _lim_CAM_front   = MyShip::_lim_front   - (PROPERTY(GAME_BUFFER_WIDTH)*LEN_UNIT/2)*revise;
    _lim_CAM_behaind = MyShip::_lim_behaind + (PROPERTY(GAME_BUFFER_WIDTH)*LEN_UNIT/2)*revise;
    _lim_CAM_zleft   = MyShip::_lim_zleft   - (PROPERTY(GAME_BUFFER_WIDTH)*LEN_UNIT/2)*revise;
    _lim_CAM_zright  = MyShip::_lim_zright  + (PROPERTY(GAME_BUFFER_WIDTH)*LEN_UNIT/2)*revise;

    _lim_VP_top     = MyShip::_lim_top     - (PROPERTY(GAME_BUFFER_HEIGHT)*LEN_UNIT/2)*revise;
    _lim_VP_bottom  = MyShip::_lim_bottom  + (PROPERTY(GAME_BUFFER_HEIGHT)*LEN_UNIT/2)*revise;
    _lim_VP_front   = MyShip::_lim_front   - (PROPERTY(GAME_BUFFER_WIDTH)*LEN_UNIT/2)*revise;
    _lim_VP_behaind = MyShip::_lim_behaind + (PROPERTY(GAME_BUFFER_WIDTH)*LEN_UNIT/2)*revise;
    _lim_VP_zleft   = MyShip::_lim_zleft   - (PROPERTY(GAME_BUFFER_WIDTH)*LEN_UNIT/2)*revise;
    _lim_VP_zright  = MyShip::_lim_zright  + (PROPERTY(GAME_BUFFER_WIDTH)*LEN_UNIT/2)*revise;
}
void VamSysCamWorker::initialize() {
    ...
    _correction_width = 0;//(PROPERTY(GAME_BUFFER_WIDTH)*LEN_UNIT/2)/4;
    _correction_height = 0;//(PROPERTY(GAME_BUFFER_HEIGHT)*LEN_UNIT/2)/4;
...
void VamSysCamWorker::processBehavior() {
    ...
    static int Dx = (int)((PROPERTY(GAME_BUFFER_WIDTH)*LEN_UNIT/2)/4*2);
