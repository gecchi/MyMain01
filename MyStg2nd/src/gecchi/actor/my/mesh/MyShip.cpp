...
void MyShip::beginMoveZX(int prm_VB) {
	...
	case VB_UP_STC:
		if (_pMover ->_angVelocity_AxisRotAngle[AXIS_X] > _angRXTopVelo_MZ) {  //けっこうな勢いで正方向(順)周りに回ってた場合
			//徐々にゆるやかに
			_pMover -> setAxisRotAngleVelocityRenge(AXIS_X, _angRXTopVelo_MZ, ANGLE180);
			_pMover -> setAxisRotAngleAcceleration(AXIS_X, -1*_angRXAcce_MZ*2); //角加速は負で徐々にゆるやかに
		} else if (_pMover ->_angVelocity_AxisRotAngle[AXIS_X] < -1*_angRXTopVelo_MZ) { //けっこうな勢いで負方向(逆)周りに回ってた場合                                          //負方向に回ってる場合
			//反発して逆周り
			_pMover -> setAxisRotAngleVelocityRenge(AXIS_X, -1*ANGLE180, _angRXTopVelo_MZ);
			_pMover -> setAxisRotAngleAcceleration(AXIS_X, _angRXAcce_MZ*2); //反発する正方向に角加速、通常の２倍角加速でいく
		} else {
			...
			if (_pMover ->_angVelocity_AxisRotAngle[AXIS_X] == 0) { //停止している場合
				_pMover -> setAxisRotAngleVelocity(AXIS_X, sgn(rd)*_angRXVelo_BeginMZ); //初速
			}
			_pMover -> setAxisRotAngleAcceleration(AXIS_X, sgn(rd)*_angRXAcce_MZ); //近いほうに角加速
		}
	...
	case VB_UP_RIGHT_STC:
		if (_pMover ->_angVelocity_AxisRotAngle[AXIS_X] > _angRXTopVelo_MZ) {  //けっこうな勢いで正方向(順)周りに回ってた場合
			//徐々にゆるやかに
			_pMover -> setAxisRotAngleVelocityRenge(AXIS_X, _angRXTopVelo_MZ, ANGLE180);
			_pMover -> setAxisRotAngleAcceleration(AXIS_X, -1*_angRXAcce_MZ*2*NANAME); //角加速は負で徐々にゆるやかに
		} else if (_pMover ->_angVelocity_AxisRotAngle[AXIS_X] < -1*_angRXTopVelo_MZ) { //けっこうな勢いで負方向(逆)周りに回ってた場合                                          //負方向に回ってる場合
			//反発して逆周り
			_pMover -> setAxisRotAngleVelocityRenge(AXIS_X, -1*ANGLE180, _angRXTopVelo_MZ);
			_pMover -> setAxisRotAngleAcceleration(AXIS_X, _angRXTopVelo_MZ*2*NANAME); //反発する正方向に角加速、通常の２倍角加速でいく
		} else {
			_pMover -> setAxisRotAngleVelocityRenge(AXIS_X, -1*_angRXTopVelo_MZ, _angRXTopVelo_MZ);
			...
			_pMover -> setAxisRotAngleAcceleration(AXIS_X, sgn(rd)*_angRXTopVelo_MZ*NANAME); //近いほうに角加速
		}
	...
	case VB_DOWN_STC:
		if (_pMover ->_angVelocity_AxisRotAngle[AXIS_X] > _angRXTopVelo_MZ) { //けっこうな勢いで正方向(逆)周りに回ってた場合
			//反発して逆周り
			_pMover -> setAxisRotAngleVelocityRenge(AXIS_X, ANGLE180, -1*_angRXTopVelo_MZ);
			_pMover -> setAxisRotAngleAcceleration(AXIS_X, -1*_angRXAcce_MZ*2); //反発する負方向に角加速、通常の２倍角加速でいく
		} else if (_pMover ->_angVelocity_AxisRotAngle[AXIS_X] < -1*_angRXTopVelo_MZ) { //けっこうな勢いで負方向(順)周りに回ってた場合                                           //負方向に回ってる場合
			//徐々にゆるやかに
			_pMover -> setAxisRotAngleVelocityRenge(AXIS_X, -1*_angRXTopVelo_MZ, -1*ANGLE180);
			_pMover -> setAxisRotAngleAcceleration(AXIS_X, _angRXAcce_MZ*2); //角加速は正で徐々にゆるやかに
		} else {
			_pMover -> setAxisRotAngleVelocityRenge(AXIS_X, -1*_angRXTopVelo_MZ, _angRXTopVelo_MZ);
	...
	case VB_DOWN_RIGHT_STC:
		if (_pMover ->_angVelocity_AxisRotAngle[AXIS_X] > _angRXTopVelo_MZ) {  //けっこうな勢いで正方向(順)周りに回ってた場合
			...
			//反発して逆周り
			_pMover -> setAxisRotAngleVelocityRenge(AXIS_X, -1*ANGLE180, _angRXTopVelo_MZ);
			_pMover -> setAxisRotAngleAcceleration(AXIS_X, _angRXAcce_MZ*2*NANAME); //反発する正方向に角加速、通常の２倍角加速でいく
		} else {
			_pMover -> setAxisRotAngleVelocityRenge(AXIS_X, -1*_angRXTopVelo_MZ, _angRXTopVelo_MZ);
			rd = _pMover->getDistanceFromAxisRotAngleTo(AXIS_X, _angRXStop_MZ*NANAME, TURN_CLOSE_TO);
			if (_pMover ->_angVelocity_AxisRotAngle[AXIS_X] == 0) { //停止している場合
			...
			_pMover -> setAxisRotAngleAcceleration(AXIS_X, sgn(rd)*_angRXAcce_MZ*NANAME); //近いほうに角加速
		}
