...
void MyLaserChip::processDrawMain() {
    ...
    if (_pChip_front != NULL && _pChip_front->isActive()) {
        ...
        if (_pChip_behind == NULL) {
            ...
            potentialDx9Exception(hr, D3D_OK, "MyLaserChip::processDrawMain() SetInt(_hKind) に失敗しました。1");
        } else {
            hr = pID3DXEffect->SetInt(_hKind, 2);
            potentialDx9Exception(hr, D3D_OK, "MyLaserChip::processDrawMain() SetInt(_hKind) に失敗しました。1");
       }
        hr = pID3DXEffect->SetFloat(_hX, 1.0*_pChip_front->_X/LEN_UNIT/ PX_UNIT);
        potentialDx9Exception(hr, D3D_OK, "MyLaserChip::processDrawMain() SetFloat(_hX) に失敗しました。1");
        hr = pID3DXEffect->SetFloat(_hY, 1.0*_pChip_front->_Y/LEN_UNIT/ PX_UNIT);
        potentialDx9Exception(hr, D3D_OK, "MyLaserChip::processDrawMain() SetFloat(_hY) に失敗しました。1");
        hr = pID3DXEffect->SetFloat(_hZ, 1.0*_pChip_front->_Z/LEN_UNIT/ PX_UNIT);
        potentialDx9Exception(hr, D3D_OK, "MyLaserChip::processDrawMain() SetFloat(_hZ) に失敗しました。1");
        ...
        if (_pChip_behind == NULL) {
            hr = pID3DXEffect->SetInt(_hKind, 1);
            potentialDx9Exception(hr, D3D_OK, "MyLaserChip::processDrawMain() SetInt(_hKind) に失敗しました。2");
        } else {
            hr = pID3DXEffect->SetInt(_hKind, 3);
            potentialDx9Exception(hr, D3D_OK, "MyLaserChip::processDrawMain() SetInt(_hKind) に失敗しました。2");
        }
        hr = pID3DXEffect->SetFloat(_hX, 1.0*_X/LEN_UNIT/ PX_UNIT);
        potentialDx9Exception(hr, D3D_OK, "MyLaserChip::processDrawMain() SetFloat(_hX) に失敗しました。2");
        hr = pID3DXEffect->SetFloat(_hY, 1.0*_Y/LEN_UNIT/ PX_UNIT);
        potentialDx9Exception(hr, D3D_OK, "MyLaserChip::processDrawMain() SetFloat(_hY) に失敗しました。2");
        hr = pID3DXEffect->SetFloat(_hZ, 1.0*_Z/LEN_UNIT/ PX_UNIT);
        potentialDx9Exception(hr, D3D_OK, "MyLaserChip::processDrawMain() SetFloat(_hZ) に失敗しました。2");
