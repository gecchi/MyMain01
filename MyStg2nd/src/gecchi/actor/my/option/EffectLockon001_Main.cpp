...
void EffectLockon001_Main::onActive() {
    ...
    locateAs(_pTarget);
    _pProg->change(EffectLockon001_SCENE_PROG_FIRST_LOCK);
}
...
void EffectLockon001_Main::processBehavior() {
    ...
    if (_pProg->get() == EffectLockon001_SCENE_PROG_LOCK || _pProg->get() == EffectLockon001_SCENE_PROG_FIRST_LOCK) {
        if (getAlpha() < 1.0) {
             addAlpha(0.01);
         }
         if (_pScaler->_method[0] == NOSCALE) {
             ...
             _pProg->change(EffectLockon001_SCENE_PROG_LOCK);
         }
         if (_pTarget) {
             if (_pTarget->isActiveActor() || _pTarget->_will_activate_after_flg) {
                 ...
                 _pProg->change(EffectLockon001_SCENE_PROG_RELEASE);
             }
         } else {
             _pProg->change(EffectLockon001_SCENE_PROG_RELEASE);
         }
    ...
    if (_pProg->get() == EffectLockon001_SCENE_PROG_RELEASE) {
        _pTarget = NULL;
        addAlpha(-0.05);
...
void EffectLockon001_Main::lockon(GgafDx9GeometricActor* prm_pTarget) {
    ...
    if (_pProg->get() == EffectLockon001_SCENE_PROG_FIRST_LOCK) {

    } else if (_pProg->get() == EffectLockon001_SCENE_PROG_LOCK) {
    } else if (_pProg->get() == EffectLockon001_SCENE_PROG_RELEASE) {
        _pScaler->forceScaleRange(60000, 2000); //スケーリング・範囲
