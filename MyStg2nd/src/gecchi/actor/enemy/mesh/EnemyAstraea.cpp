...
EnemyAstraea::EnemyAstraea(const char* prm_name) : DefaultMorphMeshActor(prm_name, "4/donatu") {
    //レーザーストック
    _laser_way = 4;
    _X = 0;
    _Y = 0;
...
void EnemyAstraea::initialize() {
    ...
    for (int i = 0; i < _laser_way; i++) {
        getLordActor()->accept(KIND_ENEMY_SHOT_NOMAL, _papLaserChipDispatcher[i]->extract()); //本所属
    }
}
