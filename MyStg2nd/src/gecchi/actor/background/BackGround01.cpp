...
void BackGround01::initialize() {
    ...
    _z = 0.999999f;//たぶん最背面 （0 <= _z < 1.0）Z=(0～+1)
...
void BackGround01::processBehavior() {
    ...
    //		for (int i = 0; i < 4; i++) {
    //			for (int j = 0; j < 4; j++) {
    //				_papapChipBoard[i]->_paVertex[j].color = D3DCOLOR_ARGB(_frame_of_behaving,255,255,255);
    //			}
    //		}
    //	}
    //フェードイン
...
////
////           //自分自身が this が段階レンダに登録され無いとprocessDrawがよばれん！
////        }
////    }
}
