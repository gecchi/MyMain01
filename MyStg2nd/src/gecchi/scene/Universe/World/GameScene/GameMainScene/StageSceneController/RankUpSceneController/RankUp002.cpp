...
RankUp002::RankUp002(const char* prm_name) : RankUpScene(prm_name) {
    ...
    _pMessage = NEW LabelGecchi16Font("RankUp002Msg");
    ...
    switch (_pProg->get()) {
        ...
        case STAGESCENE_PROG_END: {
            if (_pProg->isJustChanged()) {
                _TRACE_("RankUp002::processBehavior()  STAGESCENE_PROG_ENDになりますた！");
...
void RankUp002::onCatchEvent(UINT32 prm_no, void* prm_pSource) {
    if (prm_no == EVENT_STG01_CONTROLLER_WAS_END ) {
        _TRACE_("RankUp002::onCatchEvent() STAGEXXCONTROLLER_ENDING をキャッチ。ステータスをSTAGESCENE_PROG_ENDへ");
