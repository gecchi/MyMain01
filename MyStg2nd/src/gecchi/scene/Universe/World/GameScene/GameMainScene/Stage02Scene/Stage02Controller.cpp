...
void Stage02Controller::initialize() {
    ...
    _pProg->change(STAGE02CONTROLLER_SCENE_PROG_INIT);
}
...
void Stage02Controller::processBehavior() {
	...
	if (getActivePartFrame() == _paFrame_NextEvent[_iCnt_Event]) {
		switch (getActivePartFrame()) {
			...
			case 3: {
				...
				addSubLast(pScene);
				_pProg->change(STAGE02CONTROLLER_SCENE_PROG_STG02_01_BEGIN);
				break;
			}
			case 100: {
			...
			case 3000: {
				...
				addSubLast(pScene);
				_pProg->change(STAGE02CONTROLLER_SCENE_PROG_STG02_CLIMAX_BEGIN);
				break;
			}
			default :
				break;
    ...
    if (_pProg->isJustChangedTo(STAGE02CONTROLLER_SCENE_PROG_INIT)) {
        _TRACE_("Stage02Controller::processBehavior はいはいDemoさんありがとう、私も起動しましたよ");

    }

    if (_pProg->isJustChangedTo(STAGE02CONTROLLER_SCENE_PROG_STG02_01_BEGIN)) {
        //STG02_01開始処理

        _pProg->change(STAGE02CONTROLLER_SCENE_PROG_STG02_01_PLAYING);
    } else if (_pProg->get() == STAGE02CONTROLLER_SCENE_PROG_STG02_01_PLAYING) {
        //STG02_01最中の処理

    }

//    if (_pProg->isJustChangedTo(STAGE02CONTROLLER_SCENE_PROG_STG02_02_BEGIN)) {
//        //STG02_02開始処理
...
//        _pProg->change(STAGE02CONTROLLER_SCENE_PROG_STG02_02_PLAYING);
//    } else if (_pProg->get() == STAGE02CONTROLLER_SCENE_PROG_STG02_02_PLAYING) {
//        //STG02_02最中の処理
//
//    }
//
//    if (_pProg->isJustChangedTo(STAGE02CONTROLLER_SCENE_PROG_STG02_03_BEGIN)) {
//        //STG02_03開始処理
//
//        _pProg->change(STAGE02CONTROLLER_SCENE_PROG_STG02_03_PLAYING);
//    } else if (_pProg->get() == STAGE02CONTROLLER_SCENE_PROG_STG02_03_PLAYING) {
//        //STG02_03最中の処理
//
//    }

    if (_pProg->isJustChangedTo(STAGE02CONTROLLER_SCENE_PROG_STG02_CLIMAX_BEGIN)) {
        //STG02_Climax開始処理
        ...
        _pProg->change(STAGE02CONTROLLER_SCENE_PROG_STG02_CLIMAX_PLAYING);
    } else if (_pProg->get() == STAGE02CONTROLLER_SCENE_PROG_STG02_CLIMAX_PLAYING) {
        //STG02_Climax最中の処理

    }

    if (_pProg->isJustChangedTo(STAGE02CONTROLLER_SCENE_PROG_FAINAL)) {
        //STG02_Climax終焉の処理
    ...
    } else if (_pProg->get() == STAGE02CONTROLLER_SCENE_PROG_FAINAL) {
        //STG02_Climax終焉最中の処理
        _frame_prog_fainal++;
...
void Stage02Controller::onCatchEvent(UINT32 prm_no, void* prm_pSource) {
    if (prm_no == EVENT_STG02_01_WAS_BROKEN) {
        ...
        _pProg->change(STAGE02CONTROLLER_SCENE_PROG_FAINAL); //進捗をSTAGE02CONTROLLER_SCENE_PROG_FAINALに切り替える
    } else {
