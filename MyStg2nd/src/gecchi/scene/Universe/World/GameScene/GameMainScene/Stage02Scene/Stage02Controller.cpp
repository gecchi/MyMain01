...
void Stage02Controller::initialize() {
    ...
    changeProgress(STAGE02CONTROLLER_PROG_INIT);
}
...
void Stage02Controller::processBehavior() {
    ...
    if (getActivePartFrame() == _paFrame_NextEvent[_iCnt_Event]) {
        switch (getActivePartFrame()) {
            ...
            case 3:
                ...
                addSubLast(pScene);
                changeProgress(STAGE02CONTROLLER_PROG_STG02_01_BEGIN);
                }
                break;
            case 1200:
            ...
            case 3000:
                ...
                addSubLast(pScene);
                changeProgress(STAGE02CONTROLLER_PROG_STG02_CLIMAX_BEGIN);
                }
                break;
            default :
                break;
    ...
    if (onActiveProgress(STAGE02CONTROLLER_PROG_INIT)) {
        _TRACE_("Stage02Controller::processBehavior はいはいDemoさんありがとう、私も起動しましたよ");

    }

    if (onActiveProgress(STAGE02CONTROLLER_PROG_STG02_01_BEGIN)) {
        //STG02_01開始処理

        changeProgress(STAGE02CONTROLLER_PROG_STG02_01_PLAYING);
    } else if (getProgress() == STAGE02CONTROLLER_PROG_STG02_01_PLAYING) {
        //STG02_01最中の処理

    }

//    if (onActiveProgress(STAGE02CONTROLLER_PROG_STG02_02_BEGIN)) {
//        //STG02_02開始処理
...
//        changeProgress(STAGE02CONTROLLER_PROG_STG02_02_PLAYING);
//    } else if (getProgress() == STAGE02CONTROLLER_PROG_STG02_02_PLAYING) {
//        //STG02_02最中の処理
//
//    }
//
//    if (onActiveProgress(STAGE02CONTROLLER_PROG_STG02_03_BEGIN)) {
//        //STG02_03開始処理
//
//        changeProgress(STAGE02CONTROLLER_PROG_STG02_03_PLAYING);
//    } else if (getProgress() == STAGE02CONTROLLER_PROG_STG02_03_PLAYING) {
//        //STG02_03最中の処理
//
//    }

    if (onActiveProgress(STAGE02CONTROLLER_PROG_STG02_CLIMAX_BEGIN)) {
        //STG02_Climax開始処理
        ...
        changeProgress(STAGE02CONTROLLER_PROG_STG02_CLIMAX_PLAYING);
    } else if (getProgress() == STAGE02CONTROLLER_PROG_STG02_CLIMAX_PLAYING) {
        //STG02_Climax最中の処理

    }

    if (onActiveProgress(STAGE02CONTROLLER_PROG_FAINAL)) {
        //STG02_Climax終焉の処理
...
void Stage02Controller::catchEvent(UINT32 prm_no, void* prm_pSource) {
    if (prm_no == EVENT_STG02_01_WAS_BROKEN) {
        ...
        changeProgress(STAGE02CONTROLLER_PROG_FAINAL); //進捗をSTAGE02CONTROLLER_PROG_FAINALに切り替える
    } else {
