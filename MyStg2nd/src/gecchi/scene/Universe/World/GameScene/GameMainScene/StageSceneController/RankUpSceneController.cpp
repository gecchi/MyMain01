...
void RankUpSceneController::execute() {
    readyStage(_RANK_UP_LEVEL_);     //これはパスされるはずであるが、念のため。
    readyStage(_RANK_UP_LEVEL_ + 1); //次のシーンを先行予約
    _pLastRankUpScene = (RankUpScene*)obtainSceneFromFactory(ORDER_ID_RANKUP+_RANK_UP_LEVEL_);
...
void RankUpSceneController::slowdown(RankUpScene* prm_pLastAdded) {
    //スローダウン
    //P_STAGE_CONTROLLER->addRunFrameOnceTree(1);
