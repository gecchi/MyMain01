...
void GameMainScene::processBehavior() {
    ...
    _pFont8_JIKI_X->update(1, PROPERTY(GAME_BUFFER_HEIGHT) - 8*3-1, _buf);
    sprintf(_buf, "Y:%8d", P_MYSHIP->_Y);
    _pFont8_JIKI_Y->update(1, PROPERTY(GAME_BUFFER_HEIGHT) - 8*2-1, _buf);
    sprintf(_buf, "Z:%8d", P_MYSHIP->_Z);
    _pFont8_JIKI_Z->update(1, PROPERTY(GAME_BUFFER_HEIGHT) - 8*1-1, _buf);
    ...
    switch (_pPrg->get()) {
        ...
        case GAMEMAIN_SCENE_PROG_BEGIN: {
            ...
            if (_pPrg->getFrameInProgress() == 120) { //deleteを考慮し２秒遊ぶ
