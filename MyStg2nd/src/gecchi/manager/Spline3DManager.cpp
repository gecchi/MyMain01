...
//MyShip::_lim_top     = PROPERTY(GAME_BUFFER_HEIGHT)*5*LEN_UNIT / 2;      //上下は画面高さの大体５画面分
//MyShip::_lim_bottom  = -(PROPERTY(GAME_BUFFER_HEIGHT)*5*LEN_UNIT / 2);
//MyShip::_lim_front   = PROPERTY(GAME_BUFFER_WIDTH)*4*LEN_UNIT / 2 ;    //前後は画面幅の大体４画面分
//MyShip::_lim_behaind = -(PROPERTY(GAME_BUFFER_WIDTH)*0.5*LEN_UNIT / 2 );
//MyShip::_lim_zleft   = PROPERTY(GAME_BUFFER_WIDTH)*5*LEN_UNIT / 2;       //奥手前は画面幅の大体５画面分
