...
OUT_VS GgafDx9VS_DefaultBoard(
	...
	//X座標Y座標をを -1 ～ +1 に押し込める。
	out_vs.pos.x = - 1 + ((2*prm_pos.x + 2*g_transformedX) / g_view_width);
    out_vs.pos.y =   1 - ((2*prm_pos.y + 2*g_transformedY) / g_view_height);
//	out_vs.pos.x = - 1 + 2 * ((prm_pos.x + g_transformedX) / g_view_width);
//    out_vs.pos.y =   1 - 2 * ((prm_pos.y + g_transformedY) / g_view_height);
