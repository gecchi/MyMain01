...
bool VirtualButton::isPressedVirtualJoyButton(vbj prm_VBJ) {
    ...
    if (VBJ_BASE_BUTTON_01 <= vbj_base_prm && vbj_base_prm <= VBJ_BASE_BUTTON_MAX) {
        ...
        return (VirtualButton::_mapVbjBase2PressedFunc[vbj_base_prm])(player_no); //XYZ軸上下か、XYZ軸回転＋ーか、POVの方向の場合
    } else {
        return false;
    }
}
...
bool VirtualButton::isPushedDownVirtualJoyButton(vbj prm_VBJ) {
    ...
    vbj_base vbj_base_prm = VBJ_TO_BASE(prm_VBJ);
    if (VBJ_BASE_BUTTON_01 <= vbj_base_prm && vbj_base_prm <= VBJ_BASE_BUTTON_MAX) {
        return GgafDx::Input::isPushedDownJoyButton(player_no, vbj_base_prm); //rgb_buttonの場合
    } else if ( VBJ_BASE_X_POS_MINUS <= vbj_base_prm && vbj_base_prm <= VBJ_BASE_POV_RIGHT ) {
        return (VirtualButton::_mapVbjBase2PushedDownFunc[vbj_base_prm])(player_no); //XYZ軸上下か、XYZ軸回転＋ーか、POVの方向の場合
    } else {
        return false;
    }
}
...
vbj VirtualButton::getFirstPushedDownVirtualJoyButton(int prm_player_no) {
    ...
    //0～15 => VBJ_BASE_BUTTON_01 ～VBJ_BASE_BUTTON_15 / -1:何も PushedDown されていない
