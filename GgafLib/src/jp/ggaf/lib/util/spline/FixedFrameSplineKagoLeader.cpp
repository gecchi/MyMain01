...
FixedFrameSplineKagoLeader::FixedFrameSplineKagoLeader(
                                       SplineManufacture* prm_pManufacture,
                                       GgafDx::Kago* prm_pKago_target) :
        SplineLeader(prm_pManufacture, prm_pKago_target->_pActor) {
    _pKago_target = prm_pKago_target;
...
void FixedFrameSplineKagoLeader::restart() {
...
//    if (ABS(_distance_to_begin) <= PX_C(1)) {
    if ( (ucoord)(_distance_to_begin + PX_C(1)) <= (ucoord)(PX_C(2)) ) {
        //始点への距離が無い、間引く。
        //_TRACE_("＜警告＞FixedFrameSplineKagoLeader::start("<<prm_option<<") _pActor_target="<<_pActor_target->getName()<<
        ...
        _TRACE_("＜警告＞FixedFrameSplineKagoLeader::restart("<<_option<<") _pActor_target="<<_pActor_target->getName()<<
...
void FixedFrameSplineKagoLeader::behave() {
    if (_is_leading) {
        GgafDx::Kago* const pKago_target = _pKago_target;
        ...
        if ( _point_index == _pFixedFrameSplManuf->_pSpl->_rnum) {
            if (_cnt_loop == _max_loop) {
                //終了
                _is_leading = false;
                pKago_target->stopGravitationMvSequence();
                return;
            } else {
                //ループ
                _cnt_loop++;
        ...
        if (_prev_point_index != _point_index) {
            ...
            pKago_target->setVxyzMvVeloTwd(x, y, z, mv_velo);
        }
        _leading_frames++;
    }
}
