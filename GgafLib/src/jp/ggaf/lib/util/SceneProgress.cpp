...
void SceneProgress::relateSubScene(progress prm_FirstProgress, progress prm_EndProgress, DefaultScene* prm_pFirstSubScene) {
    ...
    int num = 1;
    _DTRACE_("SceneProgress::relateSubScene() シーン("<<_pScene->getName()<<")は、SceneProgressの進捗番号とサブシーンを関連付けて操作します。対応は以下の通り。");
    for (progress prog = prm_FirstProgress; prog <= prm_EndProgress; prog++, num++) {
        ...
        _DTRACE_("   進捗番号:"<<prog<<" ==> シーン:"<<pSub->getName()<<"");
        pSub = (DefaultScene*)(pSub->getNext());
    }
}

void SceneProgress::changeWithScene(progress prm_progress) {
    _DTRACE_("SceneProgress::changeWithScene("<<prm_progress<<")  進捗シーン:"<<get()<<"->"<<prm_progress<<" ");
    if (_mapProg2Scene.find(prm_progress) == _mapProg2Scene.end()) {
                _DTRACE_("＜警告＞SceneProgress::changeWithScene シーン("<<_pScene->getName()<<")で、"<<
...
void SceneProgress::changeWithSceneFlipping(progress prm_progress) {
    _DTRACE_("SceneProgress::changeWithSceneFlipping("<<prm_progress<<")  進捗シーン:"<<get()<<"->"<<prm_progress<<" ");

    if (_mapProg2Scene.find(get()) == _mapProg2Scene.end()) {
        _DTRACE_("＜警告＞SceneProgress::changeWithSceneFlipping シーン("<<_pScene->getName()<<")で、"<<
...
void SceneProgress::changeWithSceneCrossfading(progress prm_progress, frame prm_cross_fade_frames) {
    _DTRACE_("SceneProgress::changeWithSceneCrossfading("<<prm_progress<<","<<prm_cross_fade_frames<<")  進捗シーン:"<<get()<<"->"<<prm_progress<<" ");
    if (_mapProg2Scene.find(get()) == _mapProg2Scene.end()) {
        _DTRACE_("＜警告＞SceneProgress::changeWithSceneCrossfading シーン("<<_pScene->getName()<<")で、"<<
...
void SceneProgress::changeWithSceneFadeoutFadein(progress prm_progress, frame prm_fade_out, frame prm_fade_in) {
    _DTRACE_("SceneProgress::changeWithSceneFadeoutFadein("<<prm_progress<<","<<prm_fade_out<<","<<prm_fade_in<<")  進捗シーン:"<<get()<<"->"<<prm_progress<<" ");
    if (_mapProg2Scene.find(get()) == _mapProg2Scene.end()) {
        _DTRACE_("＜警告＞SceneProgress::changeWithSceneFadeoutFadein シーン("<<_pScene->getName()<<")で、"<<
            "サブシーンと関連付けされていない遷移元進捗(get()="<<get()<<")だった為、inactivateDelay() ができませんでした。");
    } else {
...
void SceneProgress::changeWithSceneOverlapping(progress prm_progress, frame prm_overlapping_frames) {
    _DTRACE_("SceneProgress::changeWithSceneOverlapping("<<prm_progress<<","<<prm_overlapping_frames<<")  進捗シーン:"<<get()<<"->"<<prm_progress<<" ");
    if (_mapProg2Scene.find(get()) == _mapProg2Scene.end()) {
        _DTRACE_("＜警告＞SceneProgress::changeWithSceneOverlapping シーン("<<_pScene->getName()<<")で、"<<
            "サブシーンと関連付けされていない遷移元進捗(get()="<<get()<<")だった為、inactivateDelay() ができませんでした。");
    } else {
    ...
    if (_count_next_promise > 0) {
        ...
        if (prm_progress == _progress_next_promise) {
            ...
            if (_mapProg2Scene.find(_progress_next_promise) == _mapProg2Scene.end()) {
                _DTRACE_("＜情報＞SceneProgress::change("<<prm_progress<<")  シーン("<<_pScene->getName()<<")で、"<<
...
void SceneProgress::changeNothing() {
    if (_count_next_promise > 0) {
        ...
        //_progress_next_promiseの activateDelay() を取り消す。
        if (_mapProg2Scene.find(_progress_next_promise) == _mapProg2Scene.end()) {
            _DTRACE_("＜情報＞SceneProgress::changeNothing() シーン("<<_pScene->getName()<<")で、"<<
                "遷移先予約進捗(_progress_next_promise="<<_progress_next_promise<<")が存在しましたが、サブシーンと関連付けされていないため、"<<
                "活動予約を取り消しの措置は行いません。");
        } else {
            _DTRACE_("＜情報＞SceneProgress::changeNothing()  シーン("<<_pScene->getName()<<")で、"<<
...
void SceneProgress::changeNext() {
    if (_count_next_promise > 0) {
        ...
        if (_progress+1 == _progress_next_promise) {
            ...
            //_progress_next_promiseの activateDelay() を取り消す。
            if (_mapProg2Scene.find(_progress_next_promise) == _mapProg2Scene.end()) {
                _DTRACE_("＜情報＞SceneProgress::changeNext() _progress+1="<<(_progress+1)<<" シーン("<<_pScene->getName()<<")で、"<<
                "遷移先予約進捗(_progress_next_promise="<<_progress_next_promise<<")が存在しましたが、サブシーンと関連付けされていないため、"<<
                "活動予約を取り消しの措置は行いません。");
            } else {
                _DTRACE_("＜情報＞SceneProgress::changeNext() _progress+1="<<(_progress+1)<<" シーン("<<_pScene->getName()<<")で、"<<
