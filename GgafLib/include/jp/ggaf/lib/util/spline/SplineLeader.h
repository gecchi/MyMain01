...
public:
     ...
     * RELATIVE_COORD_DIRECTION : 始点を現座標とし、さらに現在の向き（_pRikisha の _rz_mv, _ry_mv)でスプライン座標群をワールド変換。
     */
...
public:
     ...
     * 力車(GgafDx::Rikisha)のbehave();より先に実行して下さい。
     */
...
//     * start()時の対象アクター力車の移動方向(callRikisha()->_rz_mv, callRikisha()->_ry_mv）
