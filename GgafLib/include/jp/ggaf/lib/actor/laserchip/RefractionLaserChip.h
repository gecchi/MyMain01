 ...
 * 先頭チップは  GgafDx::VecDriver#behave() で移動します。<BR>
 * dispatch() したら、初期座標と VecDriver（速度・方向）を設定して下さい。<BR>
 * @version 1.00
 * @since 2010/01/19
...
public:
     ...
     * 内部処理は、
     * 先頭チップは、GgafDx::VecDriver#behave() を実行により移動を行います。
     * 屈折中は GgafDx::VecDriver#behave() による移動は行いません。
