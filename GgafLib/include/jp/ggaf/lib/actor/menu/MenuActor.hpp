...
public:
     ...
     * _pCursorActor->getLocusVehicle()->behave(); <BR>
     * を実行しています。したがって、引数のカーソルクラスで、<BR>
     * getLocusVehicle()->behave(); <BR>
     * を実行する必要はありません。<BR>
     ...
     * 【注意】<BR>
     * カーソル移動を制御するため、MenuActor<T>::processBehavior() 内で、<BR>
     * _lstSubCursor.getFromFirst(i)->_pActor->getLocusVehicle()->behave(); <BR>
     * を実行しています。したがって、引数のカーソルクラスで、<BR>
     * getLocusVehicle()->behave(); <BR>
     * を実行する必要はありません。<BR>
...
void MenuActor<T>::moveCursor(bool prm_smooth) {
    if (_pCursorActor) {
        ...
        GgafDx::LocusVehicle* pCursorLocusVehicle = _pCursorActor->getLocusVehicle();
        if (prm_smooth) {
            pCursorLocusVehicle->setMvAngTwd(
                                    pTargetItem->_x + _x_cursor_adjust,
                                    pTargetItem->_y + _y_cursor_adjust,
                                    pTargetItem->_z + _z_cursor_adjust
                              );
            pCursorLocusVehicle->stop();
            pCursorLocusVehicle->asstMv()->slideByDt(
            ...
            pCursorLocusVehicle->asstMv()->stopSliding();
            pCursorLocusVehicle->stop();
...
void MenuActor<T>::moveSubCursor(int prm_subcur_no, bool prm_smooth) {
    if (_lstSubCursor.length() > 0) {
        ...
        GgafDx::LocusVehicle* pSubCursorLocusVehicle = pSubCursorActor->getLocusVehicle();
        if (prm_smooth) {
            pSubCursorLocusVehicle->setMvAngTwd(
                                     pTargetItem->_x + pSubCursor->_x_adjust,
                                     pTargetItem->_y + pSubCursor->_y_adjust,
                                     pTargetItem->_z + pSubCursor->_z_adjust
                                 );
            pSubCursorLocusVehicle->stop();
            pSubCursorLocusVehicle->asstMv()->slideByDt(
            ...
            pSubCursorLocusVehicle->asstMv()->stopSliding();
            pSubCursorLocusVehicle->stop();
...
void MenuActor<T>::processBehavior() {
    ...
    if (_pCursorActor) {
        _pCursorActor->getLocusVehicle()->behave();
        //メインカーソル側で、_pLocusVehicle->behave() しないように注意
    }
    int n_sc = _lstSubCursor.length();
    for (int i = 0; i < n_sc; i++) {
        SubCursor* pSubCursor = _lstSubCursor.getFromFirst(i);
        pSubCursor->_pActor->getLocusVehicle()->behave();
        //補助カーソル側で、_pLocusVehicle->behave() しないように注意
    }
    ...
    if (_pCursorActor) {
        GgafDx::FigureActor* pTargetItem = _lstItems.getCurrent();
        if (_pCursorActor->getLocusVehicle()->asstMv()->isSliding()) {
    ...
    for (int i = 0; i < n_sc; i++) {
        ...
        GgafDx::FigureActor* pSubCursorActor = pSubCursor->_pActor;
        if (pSubCursorActor->getLocusVehicle()->asstMv()->isSliding()) {
